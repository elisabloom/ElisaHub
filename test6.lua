                    ...
                    if gui then
                        ...
                        if endFrame then
                            ...
                            if math.random(1, 20) == 1 then
                                print("[WEBHOOK TRACKER] Status check - EndFrame visible: " .. tostring(currentState) .. " | IsTracking: " .. tostring(getgenv().WebhookConfig.IsTracking))
                            end
                            
                            -- ✅ DETECTAR TRANSICIÓN DE false → true (fin de partida)
                            if currentState == true and lastEndFrameState == false and not getgenv().WebhookConfig.IsTracking then
                                ...
                                print("[WEBHOOK TRACKER] Previous state: " .. tostring(lastEndFrameState))
                                print("[WEBHOOK TRACKER] Current state: " .. tostring(currentState))
                                print("[WEBHOOK TRACKER] IsTracking: " .. tostring(getgenv().WebhookConfig.IsTracking))
                            ...
                            -- ✅ DEBUG: EndFrame no encontrado
                            if math.random(1, 40) == 1 then
                                print("[WEBHOOK TRACKER] ⚠️ GameEnd frame not found in GameGui")
                            end
                        end
                    else
                        -- ✅ DEBUG: GameGui no encontrado
                        if math.random(1, 40) == 1 then
                            print("[WEBHOOK TRACKER] ⚠️ GameGui not found in PlayerGui")
                        end
                    end
                end)
            else
                lastEndFrameState = false
                -- ✅ DEBUG: Webhook desactivado
                if math.random(1, 40) == 1 then
                    print("[WEBHOOK TRACKER] ⏸️ Webhook is disabled")
                end
            end
        end
    end)
...
local function runGraveyardV2()
    ...
    while getMoney() < 225 do task.wait(0.14) end
    if not plantWithRetry("unit_glow_ray", positions.prismleaf1, "Prismleaf 1") then
       return false
    end
    task.wait(0.11)
    while #myUnitIDs < 1 do task.wait(0.14) end
    ...
    task.wait(0.11)
    while #myUnitIDs < 6 do task.wait(0.14) end
    ...
    task.wait(0.11)
    while #myUnitIDs < 7 do task.wait(0.14) end
    ...
    task.wait(0.11)
    while #myUnitIDs < 8 do task.wait(0.14) end
    ...
    task.wait(0.11)
    while #myUnitIDs < 10 do task.wait(0.14) end
    ...
    task.wait(0.11)
    while #myUnitIDs < 11 do task.wait(0.14) end
    ...
    -- Corrupted 1
    while getMoney() < 8666 do task.wait(0.14) end
    if not plantWithRetry("unit_eyeball", positions.corrupted1, "Corrupted 1") then
       return false
    end
    task.wait(0.11)
    while #myUnitIDs < 12 do task.wait(0.14) end
    ...
    task.wait(0.11)
    while #myUnitIDs < 13 do task.wait(0.14) end
    ...
    -- Frost Golem 1
    while getMoney() < 7000 do task.wait(0.14) end
    if not plantWithRetry("unit_frost_golem", positions.frostgolem1, "Frost Golem 1") then
       return false
    end
    task.wait(0.11)
    while #myUnitIDs < 14 do task.wait(0.14) end
    ...
    task.wait(0.11)
    while #myUnitIDs < 15 do task.wait(0.14) end
    ...
    task.wait(0.11)
    while #myUnitIDs < 16 do task.wait(0.14) end
    ...
    for i = 2, 5 do
        ...
        task.wait(0.11)
        while #myUnitIDs < (16 + i - 1) do task.wait(0.14) end
...
local function getRandomPositionPath1()
    local startPos = Vector3.new(52.990478515625, -21.75, -56.06875991821289)
    local endPos = Vector3.new(51.214019775390625, -21.75, -54.29229736328125)
    local t = math.random()
...
local function getRandomPositionPath2()
    local startPos = Vector3.new(-57.39617156982422, -21.75, -56.212493896484375)
    local endPos = Vector3.new(-55.459922790527344, -21.75, -54.276248931884766)
    local t = math.random()
    local randomX = startPos.X + t * (endPos.X - startPos.X)
    local randomZ = startPos.Z + t * (endPos.Z - startPos.Z)
    local position = Vector3.new(randomX, -21.75, randomZ)
...
task.spawn(function()
    ...
    local GraveyardV2Toggle = AutoFarmTab:Toggle({
        Flag = "GraveyardV2",
        Title = "Graveyard V2",
        Desc = "Prismleaf, Black Clover, Frost Golem, Potato, Witchleaf, Earth Dragon and Corrupted Stem",
