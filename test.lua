...
MiscTab:Button({
    Title = "Enter ARG Obby Portal",
    Desc = "Press E on the golden portal",
    Callback = function()
        pcall(function()
            local prompt = workspace.Map.ARGObby.Model.Model.ARGInteract.InteractionPrompt
            
            -- Intentar fireproximityprompt primero
            local ok = pcall(function()
                fireproximityprompt(prompt)
            end)
            
            if not ok then
                -- Fallback: disparar conexiones manualmente
                local conns = getconnections(prompt.Triggered)
                if conns and #conns > 0 then
                    for _, conn in pairs(conns) do
                        pcall(function() conn:Fire(game:GetService("Players").LocalPlayer) end)
                    end
                    print("[ARG OBBY] ✓ Fired via connections")
                else
                    -- Último fallback: buscar RemoteEvent relacionado
                    local RS = game:GetService("ReplicatedStorage")
                    local argRemote = RS:FindFirstChild("RemoteFunctions") and 
                                      RS.RemoteFunctions:FindFirstChild("ARGInteract")
                    if argRemote then
                        argRemote:InvokeServer()
                        print("[ARG OBBY] ✓ Fired via remote")
                    else
                        warn("[ARG OBBY] No method worked")
                    end
                end
            else
                print("[ARG OBBY] ✓ fireproximityprompt worked")
            end
        end)
        
        WindUI:Notify({
            Title = "ARG Obby",
            Content = "Portal activated!",
            Duration = 3
        })
    end
})
