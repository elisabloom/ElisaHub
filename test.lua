...
local function runVolcano()
    ...
    local randomX = (math.random() - 0.5) * 2 * offset
    local randomZ = (math.random() - 0.5) * 2 * offset
    local glowPos = vector.create(baseX + randomX, baseY, baseZ + randomZ)
...
for attempt = 1, 50 do
    ...
    local px = base2X + random2X
    local pz = base2Z + random2Z
    local position = Vector3.new(px, base2Y, pz)
...
if not glow2Planted then
    warn("[VOLCANO] ⚠️ Glowthorn 2 failed after 50 attempts - continuing without it")
end
