...
MiscTab:Button({
    ...
    Callback = function()
        local success = pcall(function()
            ...
            -- Buscar el prompt recursivamente
            local prompt = nil
            for _, obj in pairs(argObby:GetDescendants()) do
                if obj.ClassName == "ProximityPrompt" and obj.Parent.Name == "ARGInteract" then
                    prompt = obj
                    break
                end
            end
            ...
            if not prompt then
                WindUI:Notify({
                    Title = "ARG Obby Failed",
                    Content = "Portal not found!",
                    Duration = 3
                })
                return
            end
            
            print("[ARG OBBY] Found prompt at: " .. prompt:GetFullName())
            fireproximityprompt(prompt)
            print("[ARG OBBY] ✓ Portal activated!")
        end)
