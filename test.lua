...
local function runVolcano()
    ...
    }, "Glowthorn") then return false end

    task.wait(0.15)
    local wt = 0
    while #myUnitIDs < 1 and wt < 10 do task.wait(0.2) wt = wt + 0.2 end
    if #myUnitIDs < 1 then warn("[VOLCANO] ❌ Failed to track Glowthorn") return false end
    local glowthornID = myUnitIDs[1]
    print("[VOLCANO] ✓ Glowthorn tracked ID: " .. tostring(glowthornID))

    upgradeToLevel(glowthornID, 4, glowthornCosts, "Glowthorn")

    -- ===== PASO 2: Rafflesia =====
    print("[VOLCANO] ========== PASO 2: RAFFLESIA ==========")
    ...
    print("[VOLCANO] ✓ Rafflesia upgraded")

    -- ===== PASO 3: Glowthorn 2 → Lvl 3 =====
    print("[VOLCANO] ========== PASO 3: GLOWTHORN 2 ==========")
    while getMoney() < 300 do task.wait(0.2) end
    ...
    if plantWithRetry("unit_frozen_spike", {
        ...
        if #myUnitIDs >= 3 then
            ...
            upgradeToLevel(glowthorn2ID, 4, glowthornCosts, "Glowthorn 2")
        else
            warn("[VOLCANO] ⚠️ Failed to track Glowthorn 2 - continuing without it")
        end
    else
        warn("[VOLCANO] ⚠️ Failed to plant Glowthorn 2 - continuing without it")
    end

    -- ===== PASO 4: Glowthorn 1 → Lvl 5 =====
    print("[VOLCANO] ========== PASO 4: GLOWTHORN 1 → LVL 5 ==========")
    upgradeToLevel(glowthornID, 5, glowthornCosts, "Glowthorn 1", 4)

    -- ===== PASO 5: Glowthorn 2 → Lvl 5 =====
    print("[VOLCANO] ========== PASO 5: GLOWTHORN 2 → LVL 5 ==========")
    if glowthorn2ID then
        upgradeToLevel(glowthorn2ID, 5, glowthornCosts, "Glowthorn 2", 4)
    end
