        ...
        for attempt = 1, 30 do
            ...
            if wSuccess and wResult then
                ...
                if #myUnitIDs >= windmillExpectedCount then
                    ...
                    -- Guardar para no venderlo al final
                    getgenv().AutoFarmConfig.WindmillID = windmillID
                    break
                else
                    warn("[VOLCANO V2] ⚠️ Windmill planted but not tracked (attempt " .. attempt .. ") - retrying...")
                end
            else
                print("[VOLCANO V2] Windmill attempt " .. attempt .. "/30 failed - trying new position...")
            end
            task.wait(0.5)
        end
