...
getgenv().WebhookConfig.SeedsAtStart = 0
...
-- RESTAURAR ANTI-BAN TAB
                    ...
                    if configData.AutoFarm_VolcanoActive then
...
local function sendWebhook(endFrame, isTest, seedsAtStart)
    ...
    local success, err = pcall(function()
        ...
        local seedsGained = "N/A"
        if not isTest and seedsAtStart and tonumber(seeds) then
            seedsGained = tostring(tonumber(seeds) - seedsAtStart)
            if tonumber(seedsGained) > 0 then
                seedsGained = "+" .. seedsGained
            end
        end
                    ...
                    if gui then
                        ...
                        if endFrame then
                            ...
                            if currentState == true and lastEndFrameState == false and not getgenv().WebhookConfig.IsTracking then
                                ...
                                sendWebhook(endFrame, false, getgenv().WebhookConfig.SeedsAtStart)
                            ...
                            if lastEndFrameState == true and currentState == false then
                                task.wait(3)
                                getgenv().WebhookConfig.SeedsAtStart = tonumber(getSeedsFromScreen()) or 0
                                print("[WEBHOOK TRACKER] 💰 Seeds at start: " .. getgenv().WebhookConfig.SeedsAtStart)
                            end
...
local function runVolcano()
    ...
    -- costs[1] = cost to go from lvl1 to lvl2 = 400
    -- costs[2] = cost to go from lvl2 to lvl3 = 650
    -- costs[3] = cost to go from lvl3 to lvl4 = 800
    -- costs[4] = cost to go from lvl4 to lvl5 = 23000
    ...
    local randomX = (math.random() - 0.5) * 2 * offset
    local randomZ = (math.random() - 0.5) * 2 * offset
    local glowPos = Vector3.new(baseX + randomX, baseY, baseZ + randomZ)
    local glowCFrame = CFrame.new(glowPos.X, glowPos.Y, glowPos.Z,
        -1, 0, -8.742277657347586e-08, 0, 1, 0, 8.742277657347586e-08, 0, -1)

    if not plantWithRetry("unit_frozen_spike", {
        CF = glowCFrame, Rotation = 180, Valid = true, Position = glowPos
    }, "Glowthorn 1") then return false end
    ...
    local function getRandomRaffPosition()
        local randomZ2 = -57.98112487792969 + math.random() * (-62.691322326660156 - (-57.98112487792969))
        local randomDist = 4.8013763427734375 + math.random() * (9.511573791503906 - 4.8013763427734375)
        local pos = Vector3.new(99.86353302001953, -31.318737030029297, randomZ2)
        return {
            Valid = true,
            PathIndex = 1,
            Position = pos,
    ...
    if not plantWithRetry("unit_rafflesia", getRandomRaffPosition(), "Rafflesia") then return false end
...
local base2X = 56.447662353515625
local base2Y = -31.568729400634766
local base2Z = 24.24903106689453

local glowthorn2ID = nil
local glow2Planted = false

for attempt = 1, 30 do
    if isGameEnded() then return false end

    local random2X = (math.random() - 0.5) * 2 * offset
    local random2Z = (math.random() - 0.5) * 2 * offset
    local glow2Pos = Vector3.new(base2X + random2X, base2Y, base2Z + random2Z)
    local glow2CFrame = CFrame.new(glow2Pos.X, glow2Pos.Y, glow2Pos.Z,
        -1, 0, -8.742277657347586e-08, 0, 1, 0, 8.742277657347586e-08, 0, -1)

    local success, result = pcall(function()
        return ReplicatedStorage:WaitForChild("RemoteFunctions")
            :WaitForChild("PlaceUnit")
            :InvokeServer("unit_frozen_spike", {
                CF = glow2CFrame, Rotation = 180, Valid = true, Position = glow2Pos
            })
    end)
    ...
    if success and result then
        ...
        task.wait(0.15)
        wt = 0
        while #myUnitIDs < 3 and wt < 10 do task.wait(0.2) wt = wt + 0.2 end

        if #myUnitIDs >= 3 then
            glowthorn2ID = myUnitIDs[3]
            print("[VOLCANO] ✓ Glowthorn 2 tracked ID: " .. tostring(glowthorn2ID))
            upgradeToLevel(glowthorn2ID, 3, glowthornCosts, "Glowthorn 2")
        else
            warn("[VOLCANO] ⚠️ Planted but not tracked - continuing without it")
        end
        break
    end
...
if not glow2Planted then
    warn("[VOLCANO] ⚠️ Glowthorn 2 failed after 30 attempts - continuing without it")
end
    ...
    -- ===== PASO 6: Glowthorn 2 → Lvl 5 =====
    print("[VOLCANO] ========== PASO 6: GLOWTHORN 2 → LVL 5 ==========")
    if glowthorn2ID then
        upgradeToLevel(glowthorn2ID, 5, glowthornCosts, "Glowthorn 2", 3)
    end

    -- ===== PASO 7: ESPERAR WAVE 20 =====
    print("[VOLCANO] ========== WAITING FOR WAVE 20 ==========")
    local currentWave = 0
    local wave18Detected = false

    while not wave18Detected and getgenv().AutoFarmConfig.VolcanoActive do
                ...
                for _, obj in pairs(gui:GetDescendants()) do
                    if obj:IsA("TextLabel") and obj.Visible and obj.Name == "Title" then
                        ...
                        if waveNum and waveNum ~= currentWave then
                            currentWave = waveNum
                            print("[VOLCANO] Wave: " .. currentWave)
                            if currentWave >= 20 then
                                print("[VOLCANO] ✓✓✓ WAVE 20!")
                                wave18Detected = true
                            end
                        end
                    end
    ...
    if not getgenv().AutoFarmConfig.VolcanoActive then return false end
    if isGameEnded() then return true end
    ...
    if glowthorn2ID then
        task.wait(0.05 + math.random() * 0.10)
        pcall(function()
            ReplicatedStorage:WaitForChild("RemoteFunctions"):WaitForChild("SellUnit"):InvokeServer(glowthorn2ID)
        end)
    end
...
local function runVolcanoV2()
    print("[VOLCANO V2] Starting Treasure Trap + Rafflesia + Chicken Coop strategy...")
    ...
    local treasurePos1 = {x = 149.4266815185547, y = -31.3182373046875, z = -53.67570877075195, dist = 67.34064301807794}
    local treasurePos2 = {x = 153.421630859375,  y = -31.31800079345703, z = -49.07331085205078, dist = 73.43864429552329}

    local raffPos1 = {x = 99.86353302001953, y = -31.318737030029297, z = -57.98112487792969,    dist = 4.8013763427734375}
    local raffPos2 = {x = 99.86353302001953, y = -31.318737030029297, z = -62.691322326660156,   dist = 9.511573791503906}

    local chickenPos1 = {x = 24.415313720703125, y = -31.318737030029297, z = 33.36052322387695, dist = 285.15735590311994}
    local chickenPos2 = {x = 28.09197425842285,  y = -31.318737030029297, z = 32.0223274230957,  dist = 281.2447360791503}
    ...
    local function getRandomTreasurePosition()
        ...
        local dist = lerpRandom(treasurePos1.dist, treasurePos2.dist)
        return {
            Valid = true,
            PathIndex = 1,
            Position = pos,
    ...
    local function getRandomRaffPosition()
        ...
        local dist = lerpRandom(raffPos1.dist, raffPos2.dist)
        return {
            Valid = true,
            PathIndex = 1,
            Position = pos,
    ...
    local function getRandomChickenPosition()
        ...
        local dist = lerpRandom(chickenPos1.dist, chickenPos2.dist)
        return {
            Valid = true,
            PathIndex = 1,
            Position = pos,
    ...
    local function getWindmillPosition()
        local rx = (math.random() - 0.5) * 2 * offset
        local rz = (math.random() - 0.5) * 2 * offset
        local baseX = 139.9478759765625
        local baseY = -31.568737030029297
        local baseZ = -77.45063781738281
    ...
    local function plantWithRetryRandom(unitName, getPositionFn, unitDisplayName, unitCount)
        for attempt = 1, 30 do
            if isGameEnded() then return false end
            ...
            if success and result then
                print("[VOLCANO V2] ✓ Planted " .. unitDisplayName .. " on attempt " .. attempt)

                task.wait(0.15)
                local wt = 0
                while #myUnitIDs < unitCount and wt < 10 do task.wait(0.2) wt = wt + 0.2 end

                if #myUnitIDs >= unitCount then
                    return myUnitIDs[unitCount]
                else
        ...
        warn("[VOLCANO V2] ❌ FAILED to plant " .. unitDisplayName)
        return nil
    end

    -- ===== PASO 1: Treasure Trap =====
    print("[VOLCANO V2] ========== PASO 1: TREASURE TRAP ==========")
    if not waitForMoney(550, 60) then return false end

    local treasureID = plantWithRetryRandom("unit_sea_chest", getRandomTreasurePosition, "Treasure Trap", 1)
    if not treasureID then
        warn("[VOLCANO V2] ⚠️ Treasure Trap failed - continuing without it")
    else
        print("[VOLCANO V2] ✓ Treasure Trap tracked ID: " .. tostring(treasureID))
    end
    ...
    local raffID = plantWithRetryRandom("unit_rafflesia", getRandomRaffPosition, "Rafflesia", 2)
    if not raffID then return false end
    print("[VOLCANO V2] ✓ Rafflesia tracked ID: " .. tostring(raffID))

    -- ===== PASO 3: Upgrade Rafflesia → Lvl 2 =====
    ...
    -- ===== PASO 3.5: VENDER TREASURE TRAP Y PLANTAR WINDMILL =====
    local windmillID = nil

    if treasureID then
        print("[VOLCANO V2] ========== PASO 3.5: SELL TREASURE TRAP + PLANT WINDMILL ==========")
        ...
        -- Plantar Windmill con sistema de retry dedicado
        local nextCount = #myUnitIDs + 1
        windmillID = nil
        if not waitForMoney(5000, 60) then
            warn("[VOLCANO V2] ⚠️ Not enough money for Windmill - skipping")
        else
            for windmillAttempt = 1, 20 do
                if isGameEnded() then break end
                local wPos = getWindmillPosition()
                local wSuccess, wResult = pcall(function()
                    return ReplicatedStorage:WaitForChild("RemoteFunctions")
                        :WaitForChild("PlaceUnit")
                        :InvokeServer("unit_windmill", wPos)
                end)
                if wSuccess and wResult then
                    print("[VOLCANO V2] ✓ Windmill planted on attempt " .. windmillAttempt)
                    task.wait(0.15)
                    local wt = 0
                    while #myUnitIDs < nextCount and wt < 8 do task.wait(0.2) wt = wt + 0.2 end
                    if #myUnitIDs >= nextCount then
                        windmillID = myUnitIDs[nextCount]
                        print("[VOLCANO V2] ✓ Windmill tracked ID: " .. tostring(windmillID))
                        break
                    else
                        warn("[VOLCANO V2] ⚠️ Windmill planted but not tracked (attempt " .. windmillAttempt .. ") - retrying...")
                    end
                else
                    print("[VOLCANO V2] Windmill attempt " .. windmillAttempt .. "/20 failed - trying new position...")
                end
                task.wait(0.05)
            end
        end
        if not windmillID then
            warn("[VOLCANO V2] ⚠️ Windmill failed after 20 attempts - continuing without it")
        end
    else
        -- Si no había treasure trap, igual intentar plantar windmill
        print("[VOLCANO V2] ========== PASO 3.5: PLANT WINDMILL (no treasure to sell) ==========")
        local nextCount = #myUnitIDs + 1
        windmillID = nil
        if not waitForMoney(5000, 60) then
            warn("[VOLCANO V2] ⚠️ Not enough money for Windmill - skipping")
    ...
    -- ===== PASO 4: Chicken Coop → Lvl 5 =====
    print("[VOLCANO V2] ========== PASO 4: CHICKEN COOP ==========")
    if not waitForMoney(9500, 60) then return false end

    local chickenCosts = {15000, 30500, 56000, 110500}
    local nextCount = #myUnitIDs + 1
    local chickenID = plantWithRetryRandom("unit_chicken_coop", getRandomChickenPosition, "Chicken Coop", nextCount)
    if not chickenID then
        warn("[VOLCANO V2] ⚠️ Chicken Coop failed - continuing without it")
    else
        print("[VOLCANO V2] ✓ Chicken Coop tracked ID: " .. tostring(chickenID))
        upgradeToLevel(chickenID, 5, chickenCosts, "Chicken Coop")
    end

    -- ===== PASO 5: ESPERAR WAVE 20 =====
    print("[VOLCANO V2] ========== WAITING FOR WAVE 20 ==========")
    local currentWave = 0
    local wave18Detected = false
    ...
    -- ===== PASO 6: VENDER TODO MENOS WINDMILL =====
    print("[VOLCANO V2] ========== SELLING ALL (except Windmill) ==========")
    task.wait(1.0 + math.random() * 0.4)
...
-- ==================== FUNCIONES HELPER ====================
    ...
    local function loadCompleteConfig(configName)
        ...
        local autoFarmToActivate = nil
        if configData.AutoFarm_VolcanoActive then autoFarmToActivate = "Volcano" end
