...
local function startRecordingHook()
    if getgenv().MacroConfig.NamecallHook then return end -- ya activo
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        if method == "InvokeServer" and getgenv().MacroConfig.IsRecording then
            ...
            if name == "PlaceUnit" or name == "UpgradeUnit" or name == "SellUnit" or name == "ActivateUnitAbility" then
                ...
                if name == "PlaceUnit" then
                    ...
                    if type(args[2]) == "table" then
                        ...
                        action.Rotation = args[2].Rotation or 0
                        action.Valid = args[2].Valid or true
                    end
                    -- Calcular costo = Cash antes - Cash después (se actualiza tras InvokeServer)
                    local cashBefore = action.Cash
                    local result = oldNamecall(self, ...)
                    action.Cost = cashBefore - (LocalPlayer:GetAttribute("Cash") or 0)
                    table.insert(getgenv().MacroConfig.CurrentMacro, action)
                    print("[MACRO REC] Place: " .. action.UnitName .. " | Cost: $" .. action.Cost .. " | Time: " .. string.format("%.2f", timeDelta) .. "s")
                    return result
                ...
                elseif name == "SellUnit" then
                    ...
                    local cashBefore = action.Cash
                    local result = oldNamecall(self, ...)
                    action.Cost = (LocalPlayer:GetAttribute("Cash") or 0) - cashBefore -- negativo = ganancia
                    table.insert(getgenv().MacroConfig.CurrentMacro, action)
...
local function stopRecordingHook()
    if not getgenv().MacroConfig.NamecallHook then return end
    local mt = getrawmetatable(game)
    setreadonly(mt, false)
    mt.__namecall = getgenv().MacroConfig.OldNamecall
    setreadonly(mt, true)
    getgenv().MacroConfig.NamecallHook = nil
    getgenv().MacroConfig.OldNamecall = nil
    print("[MACRO REC] Hook removido")
end
    ...
    task.spawn(function()
        ...
        for i, action in ipairs(macroData.Actions) do
            ...
            if action.Type == "PlaceUnit" and action.CF then
                ...
                pcall(function()
                    PlaceUnitRemote:InvokeServer(action.UnitName, {
                        CF = adjCF,
                ...
                print("[MACRO PLAY] Place: " .. action.UnitName)
                task.wait(0.15)
