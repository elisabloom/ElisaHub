...
-- ==================== VOLCANO: GLOWTHORN + AXOLOTL HIDEOUT + WINDMILL ====================
local function runVolcano()
    print("[VOLCANO] Starting Glowthorn + Axolotl Hideout + Windmill strategy...")

    local myUnitIDs = getgenv().GlobalTracking.unitIDs
    local offset = getgenv().AntiBanConfig.PlacementOffset or 1.5

    local function upgradeToLevel(unitRef, targetLevel, costs, unitName, startLevel)
        startLevel = startLevel or 1
        -- Resolver referencia: acepta tanto ID numérico como model Instance
        local resolvedID = unitRef
        if type(unitRef) == "userdata" then
            for _, v in ipairs(unitRef:GetDescendants()) do
                if (v:IsA("IntValue") or v:IsA("NumberValue") or v:IsA("StringValue")) and
                   string.find(string.lower(v.Name), "id") then
                    resolvedID = v.Value
                    break
                end
            end
            for attrName, attrValue in pairs(unitRef:GetAttributes()) do
                if string.find(string.lower(attrName), "id") then
                    resolvedID = attrValue
                    break
                end
            end
            if type(resolvedID) == "userdata" then
                warn("[UPGRADE] No se pudo resolver ID para " .. unitName)
                return false
            end
        end
        for level = (startLevel + 1), targetLevel do
            ...
            pcall(function()
                ReplicatedStorage:WaitForChild("RemoteFunctions"):WaitForChild("UpgradeUnit"):InvokeServer(resolvedID)
            end)
    ...
    local axolotlCosts = {13000, 27000, 54000, 105000}

    -- ===== FUNCIONES DE POSICIÓN =====
    local function getGlowthornPlacement()
        local baseX = 107.95842742919922
        local baseY = -31.568737030029297
        local baseZ = -62.402870178222656
    ...
    local function getAxolotlPlacement()
        local baseX = 114.3714599609375
        local baseY = -31.568737030029297
        local baseZ = -86.36691284179688
        local rx = (math.random() - 0.5) * 2 * offset
        local rz = (math.random() - 0.5) * 2 * offset
        local pos = vector.create(baseX + rx, baseY, baseZ + rz)
        return {
    ...
    local function getWindmillPlacement()
        local baseX = 139.49594116210938
        local baseY = -31.568737030029297
        local baseZ = -78.63174438476562
        local rx = (math.random() - 0.5) * 2 * offset
        local rz = (math.random() - 0.5) * 2 * offset
        local pos = vector.create(baseX + rx, baseY, baseZ + rz)
        return {
    ...
    -- Función genérica de plantar con retry + tracking ROBUSTO (3 capas)
    local function plantAndTrack(unitName, getPositionFn, unitDisplayName, expectedCount)
        local function findUnitByOwnerIndex(index)
            local found = {}
            local mapEntities = Workspace:FindFirstChild("Map")
            if not mapEntities then return nil end
            local entities = mapEntities:FindFirstChild("Entities")
        ...
        local function getIDFromModel(model)
        ...
        -- Contar unidades propias ANTES de plantar
        local countBefore = #findUnitByOwnerIndex and 0 or 0
        pcall(function()
            local found = {}
            local mapEntities = Workspace:FindFirstChild("Map")
            if mapEntities then
                ...
                if entities then
                    for _, entity in pairs(entities:GetChildren()) do
                        if entity:IsA("Model") and entity:GetAttribute("Owner") == LocalPlayer.UserId then
                            table.insert(found, entity)
                        end
                    end
                end
        ...
        print("[PLANT] Unidades propias antes de plantar " .. unitDisplayName .. ": " .. countBefore)
        ...
        for attempt = 1, 50 do
            ...
            if success and result then
                print("[VOLCANO] ✓ Planted " .. unitDisplayName .. " (attempt " .. attempt .. ")")
                -- Esperar que aparezca en workspace (countBefore + 1 unidades)
                local targetCount = countBefore + 1
                local wt = 0
                ...
                while wt < 10 do
                    ...
                    if #found >= targetCount then
                        foundModel = found[targetCount]
                        break
                    end
                    task.wait(0.2)
                    wt = wt + 0.2
                ...
                if foundModel then
                    local id = getIDFromModel(foundModel)
                    if not id then
                        -- Intentar desde tracker global como fallback
                        task.wait(1.2)
                        id = myUnitIDs[#myUnitIDs]
                    end
                    if id then
                        print("[VOLCANO] ✓ Tracked " .. unitDisplayName .. " ID: " .. tostring(id))
                        return id
                    end
                    warn("[VOLCANO] ⚠️ Sin ID pero model existe - usando model para " .. unitDisplayName)
                    return foundModel
                end
                warn("[VOLCANO] ⚠️ Planta confirmada pero unit no apareció en workspace - reintentando...")
            else
    ...
    -- ===== PASO 1: Glowthorn → Lvl 4 =====
    print("[VOLCANO] ========== PASO 1: GLOWTHORN → LVL 4 ==========")
    if not waitForMoney(300, 60) then return false end

    local glowthornID = plantAndTrack("unit_frozen_spike", getGlowthornPlacement, "Glowthorn", 1)
    if not glowthornID then return false end

    upgradeToLevel(glowthornID, 4, glowthornCosts, "Glowthorn")

    -- ===== PASO 1B: Segundo Glowthorn → Lvl 3 =====
    print("[VOLCANO] ========== PASO 1B: SEGUNDO GLOWTHORN → LVL 3 ==========")
    if not waitForMoney(300, 60) then return false end

    local function getGlowthorn2Placement()
        local baseX = 48.38316345214844
        local baseY = -31.56873321533203
        local baseZ = 19.84261703491211
        local rx = (math.random() - 0.5) * 2 * offset
        local rz = (math.random() - 0.5) * 2 * offset
        local pos = vector.create(baseX + rx, baseY, baseZ + rz)
        return {
    ...
    local glowthorn2ID = plantAndTrack("unit_frozen_spike", getGlowthorn2Placement, "Glowthorn 2", 2)
    if not glowthorn2ID then
        warn("[VOLCANO] ⚠️ Glowthorn 2 failed - continuing without it")
    else
        print("[DEBUG] GlowthornID: " .. tostring(glowthornID) .. " | Glowthorn2ID: " .. tostring(glowthorn2ID))
        upgradeToLevel(glowthorn2ID, 3, glowthornCosts, "Glowthorn 2", 1)
    end

    -- ===== PASO 2: Plantar Axolotl Hideout =====
    print("[VOLCANO] ========== PASO 2: PLANTAR AXOLOTL HIDEOUT ==========")
    -- Esperar a que el tracker global termine de procesar todas las unidades anteriores
    task.wait(1.5)
    if not waitForMoney(8000, 60) then return false end

    local axolotlExpectedCount = #myUnitIDs + 1
    print("[DEBUG AXOLOTL] myUnitIDs antes de plantar: " .. #myUnitIDs .. " | esperando index: " .. axolotlExpectedCount)
    local axolotlID = plantAndTrack("unit_axolotl_cave", getAxolotlPlacement, "Axolotl Hideout", axolotlExpectedCount)
    if not axolotlID then return false end

    -- ===== PASO 3: Plantar Windmill =====
    print("[VOLCANO] ========== PASO 3: PLANTAR WINDMILL ==========")
    task.wait(1.5)
    if not waitForMoney(5000, 60) then return false end

    local windmillExpectedCount = #myUnitIDs + 1
    print("[DEBUG WINDMILL] myUnitIDs antes de plantar: " .. #myUnitIDs .. " | esperando index: " .. windmillExpectedCount)
    local windmillID = plantAndTrack("unit_windmill", getWindmillPlacement, "Windmill", windmillExpectedCount)
    if not windmillID then
        warn("[VOLCANO] ⚠️ Windmill failed - continuing without it")
    else
        getgenv().AutoFarmConfig.WindmillID = windmillID
    end

    -- ===== PASO 4: Axolotl Hideout → Lvl 5 =====
    print("[VOLCANO] ========== PASO 4: AXOLOTL HIDEOUT → LVL 5 ==========")
    upgradeToLevel(axolotlID, 5, axolotlCosts, "Axolotl Hideout")

    -- ===== PASO 5: Glowthorn → Lvl 5 =====
    print("[VOLCANO] ========== PASO 5: GLOWTHORN → LVL 5 ==========")
    upgradeToLevel(glowthornID, 5, glowthornCosts, "Glowthorn", 4)

    -- ===== ESPERAR WAVE 18 =====
    print("[VOLCANO] ========== WAITING FOR WAVE 18 ==========")
    local currentWave = 0
    ...
    -- ===== VENDER TODO MENOS WINDMILL =====
    print("[VOLCANO] ========== SELLING (except Windmill) ==========")
    task.wait(1.0 + math.random() * 0.4)

    local function safeInvokeID(unitRef, label)
        ...
        pcall(function()
            ReplicatedStorage:WaitForChild("RemoteFunctions"):WaitForChild("SellUnit"):InvokeServer(id)
        end)
        print("[VOLCANO] ✓ " .. label .. " sold")
        task.wait(0.05 + math.random() * 0.10)
    end

    safeInvokeID(axolotlID,   "Axolotl Hideout")
    if glowthorn2ID then safeInvokeID(glowthorn2ID, "Glowthorn 2") end
    safeInvokeID(glowthornID, "Glowthorn 1")

    print("[VOLCANO] ========== SELL COMPLETE (Windmill kept) ==========")
    return true
end
...
local function startAutoFarmLoop(strategyFunction, strategyName)
    task.spawn(function()
        ...
        while getgenv().AutoFarmConfig.IsRunning and getgenv().AutoFarmConfig.VolcanoActive do
...
local VolcanoToggle = AutoFarmTab:Toggle({
    Flag = "Volcano",
    Title = "Volcano",
    Desc = "Glowthorn + Axolotl Hideout + Seedmill",
