...
local function runVolcanoV2()
    ...
    if treasureID then
        ...
        -- Plantar Windmill (será la unidad #2 en el tracking ahora que treasure fue vendida)
        -- Usamos el count actual de myUnitIDs + 1
        local nextCount = #myUnitIDs + 1
        windmillID = plantWithRetryRandom("unit_windmill", getWindmillPosition, "Windmill", nextCount)
        if windmillID then
            print("[VOLCANO V2] ✓ Windmill tracked ID: " .. tostring(windmillID))
        else
            warn("[VOLCANO V2] ⚠️ Windmill failed to plant - continuing without it")
        end
    else
