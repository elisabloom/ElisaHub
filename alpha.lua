...
local function getTotalWins()
    local success, result = pcall(function()
        ...
        for _, obj in pairs(gui:GetDescendants()) do
            if obj:IsA("TextLabel") and obj.Name == "Amount" and obj.Parent.Name == "TopBar" then
                local txt = obj.Text
                local wins = txt:match("Wins:%s*(%d+)")
                if wins then
                    return wins
                end
            end
...
local function sendHook(endFrame)
    local success, err = pcall(function()
        ...
        local description = string.format("User: %s\nSeed: %s\nCandy: %s\nRun Time: %s\nResult: %s\nTotal Wins: %s", 
            userName, seeds, candy, runTime, result, totalWins)
        ...
        warn("[WEBHOOK] Sent! Result: " .. result .. " | Seeds: " .. seeds .. " | Candy: " .. candy .. " | Time: " .. runTime .. " | Total Wins: " .. totalWins)
    end)
