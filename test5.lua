...
-- RESTAURAR ANTI-BAN TAB
                    ...
                    if autoFarmToActivate then
                        ...
                        if autoFarmToActivate == "GraveyardV1" and getgenv().AutoFarmToggles and getgenv().AutoFarmToggles.GraveyardV1 then
                            getgenv().AutoFarmToggles.GraveyardV1:Set(true)
                            print("[AUTO LOAD] Auto-activating Graveyard V1")
                        elseif autoFarmToActivate == "GraveyardV2" and getgenv().AutoFarmToggles and getgenv().AutoFarmToggles.GraveyardV2 then
                            getgenv().AutoFarmToggles.GraveyardV2:Set(true)
                            print("[AUTO LOAD] Auto-activating Graveyard V2")
                        elseif autoFarmToActivate == "Dojo" and getgenv().AutoFarmToggles and getgenv().AutoFarmToggles.Dojo then
                            getgenv().AutoFarmToggles.Dojo:Set(true)
                            print("[AUTO LOAD] Auto-activating Dojo")
                        elseif autoFarmToActivate == "AutoWinV1" and getgenv().AutoFarmToggles and getgenv().AutoFarmToggles.AutoWinV1 then
                            getgenv().AutoFarmToggles.AutoWinV1:Set(true)
                            print("[AUTO LOAD] Auto-activating Auto Win V1")
                        elseif autoFarmToActivate == "AutoWinV2" and getgenv().AutoFarmToggles and getgenv().AutoFarmToggles.AutoWinV2 then
                            getgenv().AutoFarmToggles.AutoWinV2:Set(true)
                            print("[AUTO LOAD] Auto-activating Auto Win V2")
                        end
                    end
...
task.spawn(function()
    ...
    -- ✅ GUARDAR REFERENCIA GLOBAL
    getgenv().AutoFarmToggles.GraveyardV1 = GraveyardV1Toggle
...
task.spawn(function()
    ...
    -- ==================== FUNCIONES PARA GUARDAR/CARGAR CONFIGS ====================
...
-- MAIN TAB - DROPDOWN VALUES
        ...
        -- ANTI-BAN TAB
            ...
            Webhook_Enabled = getgenv().WebhookConfig.Enabled
        }
        ...
        if not isfolder("NoahScriptHub/Configs") then
    ...
    local function loadCompleteConfig(configName)
        ...
        -- ✅ RESTAURAR AUTO FARM TAB (SOLO VALORES, NO ACTIVAR TOGGLES)
        getgenv().AutoFarmConfig.GraveyardV1Active = configData.AutoFarm_GraveyardV1Active or false
        getgenv().AutoFarmConfig.GraveyardV2Active = configData.AutoFarm_GraveyardV2Active or false
        getgenv().AutoFarmConfig.DojoActive = configData.AutoFarm_DojoActive or false
        getgenv().AutoFarmConfig.AutoWinV1Active = configData.AutoFarm_AutoWinV1Active or false
        ...
        -- ✅ APLICAR AUTO FARM TOGGLE (SI ALGUNO ESTABA ACTIVO)
        -- ⚠️ IMPORTANTE: Solo activar UNO a la vez
        local autoFarmToActivate = nil

        if getgenv().AutoFarmConfig.GraveyardV1Active then
            autoFarmToActivate = "GraveyardV1"
        elseif getgenv().AutoFarmConfig.GraveyardV2Active then
            autoFarmToActivate = "GraveyardV2"
        elseif getgenv().AutoFarmConfig.DojoActive then
            autoFarmToActivate = "Dojo"
        elseif getgenv().AutoFarmConfig.AutoWinV1Active then
            autoFarmToActivate = "AutoWinV1"
        elseif getgenv().AutoFarmConfig.AutoWinV2Active then
            autoFarmToActivate = "AutoWinV2"
        end

        if autoFarmToActivate then
            -- Esperar a que los toggles del Auto Farm Tab estén creados
            task.wait(1)
            
            -- Activar el farm correspondiente
            if autoFarmToActivate == "GraveyardV1" and getgenv().AutoFarmToggles and getgenv().AutoFarmToggles.GraveyardV1 then
                getgenv().AutoFarmToggles.GraveyardV1:Set(true)
                print("[CONFIG LOAD] Auto-activating Graveyard V1")
            elseif autoFarmToActivate == "GraveyardV2" and getgenv().AutoFarmToggles and getgenv().AutoFarmToggles.GraveyardV2 then
                getgenv().AutoFarmToggles.GraveyardV2:Set(true)
                print("[CONFIG LOAD] Auto-activating Graveyard V2")
            elseif autoFarmToActivate == "Dojo" and getgenv().AutoFarmToggles and getgenv().AutoFarmToggles.Dojo then
                getgenv().AutoFarmToggles.Dojo:Set(true)
                print("[CONFIG LOAD] Auto-activating Dojo")
            elseif autoFarmToActivate == "AutoWinV1" and getgenv().AutoFarmToggles and getgenv().AutoFarmToggles.AutoWinV1 then
                getgenv().AutoFarmToggles.AutoWinV1:Set(true)
                print("[CONFIG LOAD] Auto-activating Auto Win V1")
            elseif autoFarmToActivate == "AutoWinV2" and getgenv().AutoFarmToggles and getgenv().AutoFarmToggles.AutoWinV2 then
                getgenv().AutoFarmToggles.AutoWinV2:Set(true)
                print("[CONFIG LOAD] Auto-activating Auto Win V2")
            end
        end
    ...
    local function getAllConfigs()
        ...
        return success and configs or {}
    end
    
    -- ==================== INTERFAZ SIMPLE CON INPUT ====================
    ...
    local SaveConfigInput = SettingsTab:Input({
        Flag = "SaveConfigName",
        Title = "Config Name",
        Desc = "Enter name and press button to save",
        Type = "Input",
        Placeholder = "MyConfig",
        Callback = function(input)
            if input ~= "" then
                SaveConfigInput:Highlight()
            end
        end
    })
    ...
    SettingsTab:Button({
        ...
        Callback = function()
            local configName = SaveConfigInput.Value
            ...
            if success then
                ...
                print("[SETTINGS] Config '" .. configName .. "' saved")
            else
                WindUI:Notify({
                    Title = "Save Failed",
    ...
    local LoadConfigInput = SettingsTab:Input({
        Flag = "LoadConfigName",
        Title = "Config Name",
        Desc = "Enter exact config name to load",
        Type = "Input",
        Placeholder = "MyConfig",
        Callback = function(input)
            if input ~= "" then
                LoadConfigInput:Highlight()
            end
        end
    })
    ...
    SettingsTab:Button({
        ...
        Callback = function()
            local configName = LoadConfigInput.Value
            ...
            local success = loadCompleteConfig(configName)
            ...
            if success then
                ...
                print("[SETTINGS] Config '" .. configName .. "' loaded")
            else
                WindUI:Notify({
                    Title = "Load Failed",
                    Content = "Config '" .. configName .. "' not found!",
    ...
    local DeleteConfigInput = SettingsTab:Input({
        Flag = "DeleteConfigName",
        Title = "Config Name",
        Desc = "Enter exact config name to delete",
        Type = "Input",
        Placeholder = "MyConfig",
        Callback = function(input)
            if input ~= "" then
                DeleteConfigInput:Highlight()
            end
        end
    })
    ...
    SettingsTab:Button({
        Title = "Delete Config",
        Desc = "Permanently delete configuration",
        Callback = function()
            local configName = DeleteConfigInput.Value
            ...
            if success then
                WindUI:Notify({
                    ...
                    Content = "'" .. configName .. "' deleted successfully!",
                    Duration = 3
                })
                print("[SETTINGS] Config '" .. configName .. "' deleted")
            else
                WindUI:Notify({
                    Title = "Delete Failed",
                    Content = "Config '" .. configName .. "' not found!",
    ...
    SettingsTab:Button({
        ...
        Desc = "Display all saved configurations in console",
        Callback = function()
            ...
            if #allConfigs == 0 then
                ...
                print("[SETTINGS] No configs found")
            else
                WindUI:Notify({
                    Title = "Configs Listed",
                    Content = "Found " .. #allConfigs .. " config(s). Check console (F9)",
                    Duration = 3
                })
                print("[SETTINGS] ========== SAVED CONFIGURATIONS ==========")
                for i, name in ipairs(allConfigs) do
                    print("[SETTINGS] " .. i .. ". " .. name)
                end
                print("[SETTINGS] ================================================")
            end
        end
    ...
    local AutoLoadInput = SettingsTab:Input({
        Flag = "AutoLoadConfigName",
        Title = "Auto Load Config Name",
        Desc = "Config to load automatically on script start",
        Type = "Input",
        Placeholder = "MyConfig",
        Value = getAutoLoadConfig() or "",
        Callback = function(input)
            if input ~= "" then
                AutoLoadInput:Highlight()
            end
        end
    })
    ...
    SettingsTab:Button({
        Title = "Set Auto Load",
        Desc = "Enable auto-load for this config",
        Callback = function()
            local configName = AutoLoadInput.Value
            ...
            if not isfile(configPath) then
                WindUI:Notify({
                    ...
                    Content = "Config '" .. configName .. "' doesn't exist!",
                    Duration = 3
                })
                return
            end
            
            local success = setAutoLoadConfig(configName)
            
            if success then
                WindUI:Notify({
                    Title = "Auto Load Set",
                    Content = "'" .. configName .. "' will load automatically on next execution",
                    Duration = 4
                })
                print("[SETTINGS] Auto Load set to: " .. configName)
            else
                WindUI:Notify({
                    Title = "Failed",
                    Content = "Failed to set auto load",
                    Duration = 3
                })
            end
        end
    ...
    SettingsTab:Button({
        ...
        Desc = "Disable auto-load feature",
        Callback = function()
            ...
            if not currentAutoLoad then
                WindUI:Notify({
                    Title = "No Auto Load",
                    Content = "Auto Load is not currently set",
                    Duration = 3
                })
                return
            end
            ...
            if success then
                WindUI:Notify({
                    Title = "Auto Load Removed",
                    Content = "Auto Load disabled successfully",
                    Duration = 3
                })
                print("[SETTINGS] Auto Load removed")
                ...
                WindUI:Notify({
                    Title = "Failed",
                    Content = "Failed to remove auto load",
                    Duration = 3
                })
            end
        end
    ...
    SettingsTab:Button({
        Title = "Check Auto Load",
        Desc = "Show current auto-load config",
        Callback = function()
            local currentAutoLoad = getAutoLoadConfig()
            
            if currentAutoLoad then
                WindUI:Notify({
                    Title = "Auto Load Active",
                    Content = "Config: '" .. currentAutoLoad .. "'",
                    Duration = 4
                })
                print("[SETTINGS] Current Auto Load: " .. currentAutoLoad)
            else
                WindUI:Notify({
                    Title = "No Auto Load",
                    Content = "Auto Load is not currently set",
                ...
                print("[SETTINGS] No Auto Load set")
            end
        end
    })
