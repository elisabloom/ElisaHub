...
local function getCurrentMap()
    local success, result = pcall(function()
        ...
        local lobbySetMap17 = remoteFunctions:FindFirstChild("LobbySetMap_17")
        local lobbySetMaxPlayers17 = remoteFunctions:FindFirstChild("LobbySetMaxPlayers_17")
        local lobbySetMap8 = remoteFunctions:FindFirstChild("LobbySetMap_8")
        local lobbySetMaxPlayers8 = remoteFunctions:FindFirstChild("LobbySetMaxPlayers_8")
        
        if lobbySetMap17 or lobbySetMaxPlayers17 or lobbySetMap8 or lobbySetMaxPlayers8 then
            return "map_lobby"
        end
...
-- RESTAURAR ANTI-BAN TAB
                    ...
                    getgenv().AutoFarmConfig.VolcanoV1Active = configData.AutoFarm_VolcanoV1Active or false
                    ...
                    if configData.AutoFarm_VolcanoV1Active then
                        autoFarmToActivate = "VolcanoV1"
                    end
...
local function setupLobbyWithRetry(mapId, mapName, maxRetries)
    ...
    for attempt = 1, maxRetries do
        ...
        if currentMap == "map_lobby" then
            ...
            local setMapRemote = mapId == "map_hell" and "LobbySetMap_8" or "LobbySetMap_17"
            pcall(function()
                game:GetService("ReplicatedStorage")
                    :WaitForChild("RemoteFunctions")
                    :WaitForChild(setMapRemote)
                    :InvokeServer(mapId)
            end)
            ...
            if currentMap == "in_map" then
                ...
                if attempt < maxRetries then
                    ...
                    -- Salir del lobby
                    local leaveLobbyRemote = mapId == "map_hell" and "LeaveLobby_8" or "LeaveLobby_17"
                    pcall(function()
                        game:GetService("ReplicatedStorage")
                            :WaitForChild("RemoteFunctions")
                            :WaitForChild(leaveLobbyRemote)
                            :InvokeServer()
                    end)
        ...
        Callback = function(state)
            if state then
                ...
                local success, err = pcall(function()
                    ...
                    local success2 = game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("LobbySetMap_17"):InvokeServer(unpack(args))
...
-- ==================== VOLCANO V1: TREASURE TRAP + SEEDMILL + AXOLOTL HIDEOUT ====================
local function runVolcanoV1()
    print("[VOLCANO V1] Starting Treasure Trap + Seedmill + Axolotl Hideout strategy...")

    local myUnitIDs = getgenv().GlobalTracking.unitIDs

    local function plantTreasureTrapRandom(maxAttempts)
        maxAttempts = maxAttempts or 200

        local pos1 = {
            Valid = true,
            PathIndex = 1,
            Position = Vector3.new(154.19876098632812, -31.318031311035156, -48.12631607055664),
            DistanceAlongPath = 74.66419181711738,
            CF = CFrame.new(154.19876098632812, -31.318031311035156, -48.12631607055664,
                0.8165466785430908, 0, 0.5772794485092163,
                -0, 1, -0,
                -0.5772795081138611, 0, 0.8165465593338013),
            Rotation = 180
        }
        ...
        local pos2 = {
            Valid = true,
            PathIndex = 1,
            Position = Vector3.new(158.602294921875, -31.318737030029297, -41.10620880126953),
            DistanceAlongPath = 82.95880206588986,
            CF = CFrame.new(158.602294921875, -31.318737030029297, -41.10620880126953,
                0.8644686937332153, 0, 0.5026866793632507,
                -0, 1, -0,
                -0.5026866793632507, 0, 0.8644686937332153),
            Rotation = 180
        }

        local positions = {pos1, pos2}

        print("[VOLCANO V1] Attempting to plant Treasure Trap (max " .. maxAttempts .. " attempts)")

        for attempt = 1, maxAttempts do
            if isGameEnded() then
                warn("[VOLCANO V1] Game ended - stopping Treasure Trap plant attempts")
                return false
            end

            local placementData = positions[math.random(1, 2)]

            local success, result = pcall(function()
                return ReplicatedStorage:WaitForChild("RemoteFunctions")
                    :WaitForChild("PlaceUnit")
                    :InvokeServer("unit_sea_chest", placementData)
            end)

            if success and result then
                print("[VOLCANO V1] ✓ Placed Treasure Trap on attempt " .. attempt)
                return true
            end
        ...
        warn("[VOLCANO V1] ❌ FAILED to plant Treasure Trap after " .. maxAttempts .. " attempts!")
        return false
    end

    local function plantAxolotlHideout(maxAttempts)
        maxAttempts = maxAttempts or 50

        local baseCFrame = CFrame.new(161.84738159179688, -31.568729400634766, -67.3916015625,
            -1, 0, -8.742277657347586e-08,
            0, 1, 0,
            8.742277657347586e-08, 0, -1)

        print("[VOLCANO V1] Attempting to plant Axolotl Hideout (max " .. maxAttempts .. " attempts)")

        for attempt = 1, maxAttempts do
            if isGameEnded() then
                warn("[VOLCANO V1] Game ended - stopping Axolotl Hideout plant attempts")
                return false
            end

            local placed = placeUnit("unit_axolotl_cave", baseCFrame, 180)

            if placed then
                print("[VOLCANO V1] ✓ Planted Axolotl Hideout on attempt " .. attempt)
                return true
            end
        ...
        warn("[VOLCANO V1] ❌ FAILED to plant Axolotl Hideout after " .. maxAttempts .. " attempts!")
        return false
    end

    local function plantSeedmill(maxAttempts)
        maxAttempts = maxAttempts or 50

        local baseCFrame = CFrame.new(191.13897705078125, -31.568737030029297, -51.26609802246094,
            -1, 0, -8.742277657347586e-08,
            0, 1, 0,
            8.742277657347586e-08, 0, -1)

        print("[VOLCANO V1] Attempting to plant Seedmill (max " .. maxAttempts .. " attempts)")

        for attempt = 1, maxAttempts do
            if isGameEnded() then
                warn("[VOLCANO V1] Game ended - stopping Seedmill plant attempts")
                return false
            end

            local placed = placeUnit("unit_windmill", baseCFrame, 180)

            if placed then
                print("[VOLCANO V1] ✓ Planted Seedmill on attempt " .. attempt)
                return true
            end
        ...
        warn("[VOLCANO V1] ❌ FAILED to plant Seedmill after " .. maxAttempts .. " attempts!")
        return false
    end

    -- ===== PASO 1: PLANTAR TREASURE TRAP =====
    print("[VOLCANO V1] ========== PASO 1: TREASURE TRAP ==========")
    while getMoney() < 550 do
        if isGameEnded() then return false end
        task.wait(0.2)
    end

    if not plantTreasureTrapRandom(200) then
        WindUI:Notify({ Title = "Volcano V1 Error", Content = "Failed to plant Treasure Trap", Duration = 5 })
        return false
    end
    ...
    if #myUnitIDs < 1 then
        warn("[VOLCANO V1] ❌ Failed to track Treasure Trap")
        return false
    end

    local treasureTrapID = myUnitIDs[1]
    print("[VOLCANO V1] ✓ Treasure Trap tracked (ID: " .. tostring(treasureTrapID) .. ")")

    -- ===== PASO 2: UPGRADE TREASURE TRAP A LVL 2 =====
    print("[VOLCANO V1] ========== PASO 2: UPGRADE TREASURE TRAP → LVL 2 ==========")
    while getMoney() < 3000 do
        if isGameEnded() then return false end
        task.wait(0.2)
    end

    pcall(function()
        ReplicatedStorage:WaitForChild("RemoteFunctions"):WaitForChild("UpgradeUnit"):InvokeServer(treasureTrapID)
    end)
    task.wait(0.08 + (math.random() * 0.21))
    print("[VOLCANO V1] ✓ Treasure Trap upgraded to Lvl 2")

    -- ===== PASO 3: PLANTAR AXOLOTL HIDEOUT =====
    print("[VOLCANO V1] ========== PASO 3: AXOLOTL HIDEOUT ==========")
    while getMoney() < 8000 do
        if isGameEnded() then return false end
        task.wait(0.2)
    end

    if not plantAxolotlHideout(50) then
        WindUI:Notify({ Title = "Volcano V1 Error", Content = "Failed to plant Axolotl Hideout", Duration = 5 })
        return false
    end
    ...
    if #myUnitIDs < 2 then
        warn("[VOLCANO V1] ❌ Failed to track Axolotl Hideout")
        return false
    end

    local axolotlID = myUnitIDs[2]
    print("[VOLCANO V1] ✓ Axolotl Hideout tracked (ID: " .. tostring(axolotlID) .. ")")

    -- ===== PASO 4: PLANTAR SEEDMILL =====
    print("[VOLCANO V1] ========== PASO 4: SEEDMILL ==========")
    while getMoney() < 5000 do
        if isGameEnded() then return false end
        task.wait(0.2)
    end

    if not plantSeedmill(50) then
        warn("[VOLCANO V1] ⚠️ Failed to plant Seedmill - continuing without it")
        WindUI:Notify({ Title = "Volcano V1 Warning", Content = "Seedmill failed to place - continuing...", Duration = 4 })
    else
        task.wait(0.3)
        waitTime = 0
        ...
        if #myUnitIDs >= 3 then
            print("[VOLCANO V1] ✓ Seedmill tracked (ID: " .. tostring(myUnitIDs[3]) .. ")")
        else
            warn("[VOLCANO V1] ⚠️ Seedmill placed but tracking failed - continuing")
        end
    end

    -- ===== PASO 5: UPGRADE AXOLOTL HIDEOUT DE LVL 1 A LVL 5 =====
    print("[VOLCANO V1] ========== PASO 5: UPGRADE AXOLOTL HIDEOUT → LVL 5 ==========")

    local axolotlUpgradeCosts = {13000, 27000, 54000, 105000}

    for i, cost in ipairs(axolotlUpgradeCosts) do
        if isGameEnded() then
            warn("[VOLCANO V1] Game ended - stopping Axolotl upgrades")
            return false
        end

        local targetLevel = i + 1
        print("[VOLCANO V1] Upgrading Axolotl Hideout to Lvl " .. targetLevel .. " (cost: $" .. cost .. ")...")
        ...
        pcall(function()
            ReplicatedStorage:WaitForChild("RemoteFunctions"):WaitForChild("UpgradeUnit"):InvokeServer(axolotlID)
        end)
        task.wait(0.08 + (math.random() * 0.21))
        print("[VOLCANO V1] ✓ Axolotl Hideout upgraded to Lvl " .. targetLevel)
    end

    print("[VOLCANO V1] ✓ Axolotl Hideout fully upgraded to Lvl 5!")

    -- ===== PASO 6: ESPERAR WAVE 18 =====
    print("[VOLCANO V1] ========== PASO 6: WAITING FOR WAVE 18 ==========")

    local currentWave = 0
    local wave18Detected = false

    while not wave18Detected and getgenv().AutoFarmConfig.VolcanoV1Active do
        if isGameEnded() then
            warn("[VOLCANO V1] ✅ Game ended before Wave 18 - continuing to next match")
            return true
        end
                ...
                for _, obj in pairs(gui:GetDescendants()) do
                    if obj:IsA("TextLabel") and obj.Visible and obj.Name == "Title" then
                        ...
                        if waveNum and waveNum ~= currentWave then
                            currentWave = waveNum
                            print("[VOLCANO V1] Current wave: " .. currentWave)
                            if currentWave >= 18 then
                                print("[VOLCANO V1] ✓✓✓ WAVE 18 REACHED! ✓✓✓")
                                wave18Detected = true
                            end
                        end
                    end
    ...
    if not getgenv().AutoFarmConfig.VolcanoV1Active then
        print("[VOLCANO V1] Farm stopped by user")
        return false
    end

    if isGameEnded() then
        warn("[VOLCANO V1] ✅ Game ended - continuing to next match")
        return true
    end

    -- ===== PASO 7: VENDER TREASURE TRAP Y AXOLOTL (NO SEEDMILL) =====
    print("[VOLCANO V1] ========== PASO 7: SELLING UNITS (except Seedmill) ==========")

    local randomInitialDelay = 0.15 + (math.random() * 0.25)
    print("[VOLCANO V1] Waiting " .. string.format("%.2f", randomInitialDelay) .. "s before selling...")
    task.wait(randomInitialDelay)

    pcall(function()
        ReplicatedStorage:WaitForChild("RemoteFunctions"):WaitForChild("SellUnit"):InvokeServer(treasureTrapID)
    end)
    print("[VOLCANO V1] Sold Treasure Trap")

    task.wait(0.05 + (math.random() * 0.10))

    pcall(function()
        ReplicatedStorage:WaitForChild("RemoteFunctions"):WaitForChild("SellUnit"):InvokeServer(axolotlID)
    end)
    print("[VOLCANO V1] Sold Axolotl Hideout")

    print("[VOLCANO V1] ========== SELL COMPLETE (Seedmill kept) ==========")
    return true
end
...
local function startAutoFarmLoop(strategyFunction, strategyName)
    task.spawn(function()
        ...
        while getgenv().AutoFarmConfig.IsRunning and
              getgenv().AutoFarmConfig.VolcanoV1Active
        do
            ...
            if getgenv().AntiBanConfig.AutoReturnEnabled and
               ...
               getgenv().AutoFarmConfig.MatchesPlayed >= getgenv().AntiBanConfig.MatchesBeforeReturn then
                ...
                if getgenv().MainTabToggles then
                    if getgenv().MainTabToggles.AutoSkip then getgenv().MainTabToggles.AutoSkip:Set(false) end
                    if getgenv().MainTabToggles.AutoPlayAgain then getgenv().MainTabToggles.AutoPlayAgain:Set(false) end
                    if getgenv().MainTabToggles.AutoDifficulty then getgenv().MainTabToggles.AutoDifficulty:Set(false) end
                end

                stopGlobalTracking()

                getgenv().AutoFarmConfig.IsRunning = false
                getgenv().AutoFarmConfig.VolcanoV1Active = false
...
task.spawn(function()
    ...
    local VolcanoV1Toggle = AutoFarmTab:Toggle({
        Flag = "VolcanoV1",
        Title = "Volcano V1",
        Desc = "Treasure Trap + Seedmill + Axolotl Hideout",
        Default = getgenv().AutoFarmConfig.VolcanoV1Active,
        Callback = function(state)
            if state then
                local currentMap = getCurrentMap()
                ...
                if currentMap == "map_lobby" then
                    WindUI:Notify({
                        Title = "Lobby Setup",
                        Content = "Teleporting to Volcano lobby...",
                        Duration = 3
                    })

                    task.spawn(function()
                        local success = setupLobbyWithRetry("map_hell", "Volcano", 10)
                        if success then
                            WindUI:Notify({ Title = "Setup Complete", Content = "Successfully entered Volcano map!", Duration = 3 })
                        else
                            WindUI:Notify({ Title = "Setup Failed", Content = "Could not enter Volcano map. Please try manually.", Duration = 5 })
                        end
                    end)

                    task.wait(0.1)
                    getgenv().AutoFarmConfig.VolcanoV1Active = false
                    VolcanoV1Toggle:Set(false)
                    return
                end

                getgenv().AutoFarmConfig.VolcanoV1Active = true
                getgenv().AutoFarmConfig.IsRunning = true
                getgenv().AutoFarmConfig.CurrentStrategy = "VolcanoV1"
                getgenv().AutoFarmConfig.MatchesPlayed = 0

                WindUI:Notify({ Title = "Volcano V1 Started", Content = "Treasure Trap + Seedmill + Axolotl Hideout running...", Duration = 3 })

                startAutoFarmLoop(runVolcanoV1, "Volcano V1")
                ...
                WindUI:Notify({ Title = "Volcano V1 Stopped", Content = "Auto farm disabled", Duration = 2 })
            end
        end
    })

    getgenv().AutoFarmToggles.VolcanoV1 = VolcanoV1Toggle
...
-- ==================== FUNCIONES HELPER ====================
    ...
    local function saveCompleteConfig(configName)
        ...
        local configData = {
            ...
            AutoFarm_VolcanoV1Active = getgenv().AutoFarmConfig.VolcanoV1Active,
    ...
    local function loadCompleteConfig(configName)
        ...
        local autoFarmToActivate = nil
        if configData.AutoFarm_VolcanoV1Active then autoFarmToActivate = "VolcanoV1" end
