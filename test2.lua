...
local function plantRafflesiaWithRetry(pathFunction, rafflesiaName, maxAttempts)
    ...
    for attempt = 1, maxAttempts do
        ...
        local success, result = pcall(function()
            return game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions")
...
local function runDojo()
    ...
    if shouldFailThisMatch then
        print("[DOJO TEST] ⚠️ WARNING: SIMULATING FAILURE IN MATCH " .. currentMatchNumber)
    end
    ...
    if not plantRafflesiaWithRetry(getRandomPositionPath1, "Rafflesia 1", 10) then
        warn("[DOJO] Failed to plant Rafflesia 1 after 10 attempts")
        goto skip_match
    end
    ...
    if not plantRafflesiaWithRetry(getRandomPositionPath2, "Rafflesia 2", 10) then
        warn("[DOJO] Failed to plant Rafflesia 2 after 10 attempts")
        goto skip_match
    end
    ...
    pcall(function()
        game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("UpgradeUnit"):InvokeServer(raff1ID)
    end)
    ...
    while getMoney() < 8000 do task.wait(0.2) end
    pcall(function()
        game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("UpgradeUnit"):InvokeServer(raff2ID)
    end)
    task.wait(0.4 + (math.random() * 0.59))
    ...
    pcall(function()
        game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("SellUnit"):InvokeServer(raff1ID)
    end)
    task.wait(0.05)
    pcall(function()
        game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("SellUnit"):InvokeServer(raff2ID)
    end)
    ...
    warn("[DOJO] Waiting for game end to continue...")
    warn("==========================================")
    
    -- Notificación ya mostrada en consola con warn()
