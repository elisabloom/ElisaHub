...
while getgenv().AutoFarmConfig.IsRunning and (
    ...
    -- ✅ PASO 3: Esperar a que el botón Play Again aparezca y clickearlo manualmente aquí
    -- (en lugar de depender del AutoPlayAgain toggle que puede fallar)
    print("[AUTO FARM LOOP] ⏳ Waiting for Play Again button...")
    local playAgainClicked = false
    local waitForButtonTime = 0
    local maxWaitForButton = 20 -- máximo 20 segundos esperando el botón
    ...
    while not playAgainClicked and waitForButtonTime < maxWaitForButton and getgenv().AutoFarmConfig.IsRunning do
        pcall(function()
            ...
            if gui then
                local endFrame = gui.Screen.Middle:FindFirstChild("GameEnd")
                if endFrame and endFrame.Visible then
                    -- Buscar botón de Play Again dentro del endFrame
                    for _, button in ipairs(endFrame:GetDescendants()) do
                        if button:IsA("TextButton") and button.Visible then
                            local text = string.lower(button.Text)
                            if string.find(text, "again") or string.find(text, "play") then
                                local conns = getconnections(button.MouseButton1Click)
                                if conns and #conns > 0 then
                                    ...
                                    print("[AUTO FARM LOOP] ✅ Play Again clicked!")
                                    return
                                end
                            end
                        end
    ...
    if not playAgainClicked then
        warn("[AUTO FARM LOOP] ⚠️ Could not click Play Again after " .. maxWaitForButton .. "s - trying RemoteFunction fallback")
        pcall(function()
            game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("RestartGame"):InvokeServer()
        end)
    end
