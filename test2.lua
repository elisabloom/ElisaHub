...
-- ==================== FUNCIÓN CLICK PLAY AGAIN ====================
local function clickPlayAgain()
    local maxWait = 15
    local waited = 0
    
    print("[PLAY AGAIN] Waiting for Play Again button...")
    ...
    while waited < maxWait do
        ...
        pcall(function()
            ...
            -- Ruta exacta: Screen.Middle.GameEnd.Items.Frame.Actions.Items.Again
            local againButton = gui:FindFirstChild("Screen")
                and gui.Screen:FindFirstChild("Middle")
                and gui.Screen.Middle:FindFirstChild("GameEnd")
                and gui.Screen.Middle.GameEnd:FindFirstChild("Items")
                and gui.Screen.Middle.GameEnd.Items:FindFirstChild("Frame")
                and gui.Screen.Middle.GameEnd.Items.Frame:FindFirstChild("Actions")
                and gui.Screen.Middle.GameEnd.Items.Frame.Actions:FindFirstChild("Items")
                and gui.Screen.Middle.GameEnd.Items.Frame.Actions.Items:FindFirstChild("Again")
            
            if againButton then
                print("[PLAY AGAIN] ✅ Button found! Visible=" .. tostring(againButton.Visible))
                local conns = getconnections(againButton.MouseButton1Click)
                if conns and #conns > 0 then
                    conns[1]:Fire()
                    clicked = true
                    print("[PLAY AGAIN] ✅ Click simulated!")
                end
            end
        end)
    ...
    warn("[PLAY AGAIN] ❌ Button not found after " .. maxWait .. " seconds")
    return false
end
...
local function startAutoFarmLoop(strategyFunction, strategyName)
    task.spawn(function()
        ...
        while getgenv().AutoFarmConfig.IsRunning and (
            ...
            -- ✅ SIMULAR CLICK EN PLAY AGAIN
            print("[AUTO FARM LOOP] ========== CLICKING PLAY AGAIN ==========")
            task.wait(2)
            
            local playAgainSuccess = clickPlayAgain()
            if not playAgainSuccess then
                warn("[AUTO FARM LOOP] ❌ Could not click Play Again - trying remote fallback")
                pcall(function()
