...
local function startAutoFarmLoop(strategyFunction, strategyName)
    task.spawn(function()
        ...
        -- ✅ NO activar AutoPlayAgain - el loop maneja Play Again internamente
        ...
        if getgenv().AntiBanConfig.AutoReturnEnabled and 
           ...
           getgenv().AutoFarmConfig.MatchesPlayed >= getgenv().AntiBanConfig.MatchesBeforeReturn then
            ...
            print("[AUTO FARM LOOP] 🚨 MATCH LIMIT REACHED AFTER FIRST MATCH 🚨")
        end
        ...
        while getgenv().AutoFarmConfig.IsRunning and (
            ...
            -- ✅ ESPERAR A QUE APAREZCAN LOS BOTONES (Play again / Return to lobby)
            print("[AUTO FARM LOOP] Waiting for Play Again button to appear...")
            local playAgainButton = nil
            local waitForButtons = 0
            while not playAgainButton and waitForButtons < 15 do
                pcall(function()
                    local gui = PlayerGui:FindFirstChild("GameGui")
                    if gui then
                ...
                if not playAgainButton then
                    task.wait(0.3)
                    waitForButtons = waitForButtons + 0.3
                end
            end
            
            -- ✅ PRESIONAR PLAY AGAIN ANTES DE VOTAR DIFICULTAD
            if playAgainButton then
                print("[AUTO FARM LOOP] ✓ Play Again button found - clicking...")
                local clicked = false
                for attempt = 1, 10 do
                    pcall(function()
                ...
                if not clicked then
                    -- Fallback: usar RemoteFunction directamente
                    pcall(function()
                        game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("RestartGame"):InvokeServer()
                    end)
                    print("[AUTO FARM LOOP] Play Again via RemoteFunction (fallback)")
                end
                
                -- ✅ ESPERAR A QUE DESAPAREZCA EL FRAME DE GAME END
                print("[AUTO FARM LOOP] Waiting for GameEnd frame to disappear...")
                local frameGone = false
                local waitFrameGone = 0
                while not frameGone and waitFrameGone < 10 do
                    pcall(function()
                        local gui = PlayerGui:FindFirstChild("GameGui")
                        if gui then
                            local endFrame = gui.Screen.Middle:FindFirstChild("GameEnd")
                    ...
                    if not frameGone then
                        task.wait(0.3)
                        waitFrameGone = waitFrameGone + 0.3
                    end
                end
                print("[AUTO FARM LOOP] ✓ GameEnd frame gone after " .. waitFrameGone .. "s")
            else
                warn("[AUTO FARM LOOP] ⚠️ Play Again button not found after 15s - attempting RemoteFunction")
                pcall(function()
                    game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("RestartGame"):InvokeServer()
                end)
                task.wait(2)
            ...
            -- ✅ ESPERAR A QUE EMPIECE NUEVA PARTIDA (Wave indicator aparece)
            print("[AUTO FARM LOOP] ========== WAITING FOR NEW GAME ==========")
            task.wait(1)
            
            local newGameStarted = false
            local waitTime = 0
            while not newGameStarted and waitTime < 60 do
                pcall(function()
                    -- Detectar nuevo juego buscando el indicador de wave o que GameEnd ya no esté
                    local gui = PlayerGui:FindFirstChild("GameGui")
                    ...
                    if gui then
                        local endFrame = gui.Screen.Middle:FindFirstChild("GameEnd")
                        if not endFrame or not endFrame.Visible then
                            -- Verificar que realmente estamos en una partida (WaveControls visibles)
                            if guiNoInset then
                                local waveControls = guiNoInset:FindFirstChild("Screen")
                                if waveControls then
                                    local top = waveControls:FindFirstChild("Top")
                                    if top and top.Visible then
                                        newGameStarted = true
                                    end
                                end
                            else
                                -- Fallback: si GameEnd no está visible, asumir que empezó
                                newGameStarted = true
                            end
                        end
                    end
