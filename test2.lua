...
local function startAutoFarmLoop(strategyFunction, strategyName)
    task.spawn(function()
        ...
        while getgenv().AutoFarmConfig.IsRunning and (
            ...
            -- Esperar a que desaparezca la pantalla Choose Level antes de votar
            local waitedForLevel = 0
            while waitedForLevel < 15 do
                local inMap = getCurrentMap()
                if inMap == "in_map" then
                    print("[AUTO FARM LOOP] ✓ In map confirmed - voting difficulty now")
                    break
                end
                task.wait(0.5)
                waitedForLevel = waitedForLevel + 0.5
            end
