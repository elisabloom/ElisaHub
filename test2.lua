...
while getgenv().AutoFarmConfig.IsRunning and (
    ...
    -- ✅ PASO 1: Esperar GameEnd visible
    ...
    while not gameEnded and getgenv().AutoFarmConfig.IsRunning do
        ...
        if not gameEnded then task.wait(0.5) end
    end
    ...
    if getgenv().AntiBanConfig.AutoReturnEnabled and 
       ...
       getgenv().AutoFarmConfig.MatchesPlayed >= getgenv().AntiBanConfig.MatchesBeforeReturn then
        ...
        WindUI:Notify({
            ...
            Content = "Returned to lobby after " .. getgenv().AntiBanConfig.MatchesBeforeReturn .. " matches",
            Duration = 5
        })
        return
    end
    
    -- ✅ PASO 3: Esperar 3s y ejecutar RestartGame
    print("[AUTO FARM LOOP] ⏳ Waiting 3s for end screen...")
    task.wait(3)
    
    print("[AUTO FARM LOOP] 🔄 Invoking RestartGame...")
    pcall(function()
        game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("RestartGame"):InvokeServer()
        print("[AUTO FARM LOOP] ✅ RestartGame invoked!")
    end)
    
    -- ✅ PASO 4: Esperar que GameEnd desaparezca
    print("[AUTO FARM LOOP] ⏳ Waiting for GameEnd to disappear...")
    local waitForGoneTime = 0
    while waitForGoneTime < 20 and getgenv().AutoFarmConfig.IsRunning do
        local gone = false
        pcall(function()
            local gui = PlayerGui:FindFirstChild("GameGui")
            if gui then
    ...
    -- ✅ PASO 5: Esperar que GameEnd desaparezca y haya un pequeño buffer
print("[AUTO FARM LOOP] ⏳ Waiting for GameEnd to disappear...")
local waitForGoneTime = 0
while waitForGoneTime < 20 and getgenv().AutoFarmConfig.IsRunning do
    local gone = false
...
-- ✅ PASO 6: Votar dificultad repetidamente hasta que Wave 1 aparezca
-- (el vote screen puede tardar en aparecer, por eso votamos en loop)
print("[AUTO FARM LOOP] ========== VOTING DIFFICULTY + WAITING FOR WAVE 1 ==========")
local wave1Detected = false
local waitForWaveTime = 0
local maxWaitForWave = 60
local lastVoteTime = 0

while not wave1Detected and waitForWaveTime < maxWaitForWave and getgenv().AutoFarmConfig.IsRunning do
    
    -- Votar dificultad cada 2 segundos mientras esperamos
    if (waitForWaveTime - lastVoteTime) >= 2 then
        pcall(function()
            game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("PlaceDifficultyVote"):InvokeServer(difficulty)
            print("[AUTO FARM LOOP] 🗳️ Voted difficulty: " .. difficulty .. " at " .. waitForWaveTime .. "s")
        end)
        lastVoteTime = waitForWaveTime
    end
            ...
            for _, obj in pairs(gui:GetDescendants()) do
                if obj:IsA("TextLabel") and obj.Visible and obj.Name == "Title" then
                    ...
                    if waveNum and waveNum >= 1 then
                        wave1Detected = true
                        print("[AUTO FARM LOOP] ✅ Wave " .. waveNum .. " detected - match started!")
                        return
                    end
                end
            end
    ...
    if not wave1Detected then
        task.wait(0.3)
        waitForWaveTime = waitForWaveTime + 0.3
    end
end

if not wave1Detected then
    warn("[AUTO FARM LOOP] ❌ Wave 1 never detected - stopping farm")
    stopGlobalTracking()
    WindUI:Notify({
        Title = "❌ Auto Farm Error",
        Content = "New match never started - farm stopped",
        Duration = 5
    })
    break
end

-- ✅ PASO 7: Resetear tracking y ejecutar macro
print("[AUTO FARM LOOP] 🔄 Resetting tracking for Match #" .. (getgenv().AutoFarmConfig.MatchesPlayed + 1))
resetGlobalTracking()

-- Reconectar tracking si es necesario
if not getgenv().GlobalTracking.connection then
    local mapFolder = Workspace:FindFirstChild("Map")
    if mapFolder then
        local entities = mapFolder:FindFirstChild("Entities")
        if entities then
            ...
            print("[GLOBAL TRACKING] 🔄 Reconnected for new match")
        end
    end
end
...
print("[AUTO FARM LOOP] ========== EXECUTING MACRO FOR MATCH #" .. (getgenv().AutoFarmConfig.MatchesPlayed + 1) .. " ==========")
    ...
    if macroSuccess then
        ...
        warn("[AUTO FARM LOOP] 📋 ERROR: " .. tostring(macroError))
        getgenv().AutoFarmConfig.MatchesPlayed = matchNumberAttempted
        WindUI:Notify({
            Title = "⚠️ Macro Failed",
            Content = "Error in match #" .. matchNumberAttempted .. " - continuing",
            Duration = 4
        })
    end
