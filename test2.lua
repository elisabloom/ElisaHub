        ...
        if obj:IsA("TextLabel") and obj.Visible then
            ...
            if txtLower:find("run time") then
                ...
                if secsMatch then
                    ...
                    if secs and secs < 36000 then  -- ✅ CRÍTICO: verifica que sea < 10 horas
...
local function startAutoFarmLoop(strategyFunction, strategyName)
    task.spawn(function()
        ...
        -- ✅ ACTIVAR AUTO SKIP Y ANTI-AFK (NO AutoPlayAgain ni AutoDifficulty)
        print("[AUTO FARM LOOP] Activating toggles...")
        task.wait(1)
        ...
        -- ✅ DESACTIVAR AutoDifficulty si estaba activo (el loop vota internamente)
        if getgenv().MainTabConfig.AutoDifficulty and getgenv().MainTabToggles.AutoDifficulty then
            getgenv().MainTabToggles.AutoDifficulty:Set(false)
        end
        
        -- ✅ DESACTIVAR AutoPlayAgain si estaba activo (el loop presiona internamente)
        if getgenv().MainTabConfig.AutoPlayAgain and getgenv().MainTabToggles.AutoPlayAgain then
        ...
        -- ✅ FUNCIÓN INTERNA: VOTAR DIFICULTAD
        local function voteDifficulty()
            ...
            print("[AUTO FARM LOOP] ✓ Voted difficulty: " .. difficultyName)
        end
        
        -- ✅ FUNCIÓN INTERNA: PRESIONAR PLAY AGAIN Y ESPERAR QUE INICIE EL JUEGO
        local function pressPlayAgainAndWait()
            -- Paso 1: Esperar que aparezcan los botones
            print("[AUTO FARM LOOP] Waiting for Play Again button...")
            local playAgainButton = nil
            local waited = 0
            while not playAgainButton and waited < 20 do
                pcall(function()
                    local gui = PlayerGui:FindFirstChild("GameGui")
                    if gui then
            ...
            if not playAgainButton then
                warn("[AUTO FARM LOOP] Play Again button not found - using RemoteFunction")
                pcall(function()
                    game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("RestartGame"):InvokeServer()
                end)
            else
                print("[AUTO FARM LOOP] ✓ Play Again found, clicking...")
                local clicked = false
                for attempt = 1, 10 do
                    pcall(function()
                        local conns = getconnections(playAgainButton.MouseButton1Click)
                        if conns and #conns > 0 then
                            conns[1]:Fire()
                            clicked = true
                        end
            ...
            -- Paso 2: Esperar que desaparezca el GameEnd frame
            print("[AUTO FARM LOOP] Waiting for GameEnd frame to disappear...")
            local frameGone = false
            local waitFrame = 0
            while not frameGone and waitFrame < 15 do
                pcall(function()
                    local gui = PlayerGui:FindFirstChild("GameGui")
                    if gui then
                        local endFrame = gui.Screen.Middle:FindFirstChild("GameEnd")
                ...
                if not frameGone then
                    task.wait(0.3)
                    waitFrame = waitFrame + 0.3
                end
            end
            print("[AUTO FARM LOOP] ✓ GameEnd gone after " .. waitFrame .. "s")
            
            -- Paso 3: Ahora votar dificultad (el selector aparece aquí)
            task.wait(0.5)
            voteDifficulty()
            
            -- Paso 4: Esperar que inicie la nueva partida (WaveControls visible)
            print("[AUTO FARM LOOP] Waiting for new game to start...")
            local gameStarted = false
            local waitStart = 0
            while not gameStarted and waitStart < 60 do
                pcall(function()
                    local guiNoInset = PlayerGui:FindFirstChild("GameGuiNoInset")
                    if guiNoInset then
                        local screen = guiNoInset:FindFirstChild("Screen")
                        if screen then
                            local top = screen:FindFirstChild("Top")
                            if top and top.Visible then
                ...
                if not gameStarted then
                    task.wait(0.5)
                    waitStart = waitStart + 0.5
                end
            end
            
            if gameStarted then
                print("[AUTO FARM LOOP] ✓ New game started after " .. waitStart .. "s")
            else
                warn("[AUTO FARM LOOP] ⚠️ Could not confirm game start after 60s - continuing anyway")
            end
        ...
        -- ==================== PRIMER MATCH ====================
        print("[AUTO FARM LOOP] ========== VOTING DIFFICULTY FOR FIRST MATCH ==========")
        voteDifficulty()
        task.wait(1)
        ...
        if macroSuccess then
            ...
            warn("[AUTO FARM LOOP] ❌ First macro had errors: " .. tostring(macroError))
            WindUI:Notify({
        ...
        while getgenv().AutoFarmConfig.IsRunning and (
            ...
            if getgenv().AntiBanConfig.AutoReturnEnabled and 
               ...
               getgenv().AutoFarmConfig.MatchesPlayed >= getgenv().AntiBanConfig.MatchesBeforeReturn then
                ...
                if returnSuccess then
                    ...
                    warn("[AUTO FARM LOOP] ✗ Return failed")
                end
            ...
            -- ✅ ESPERAR QUE TERMINE EL JUEGO ACTUAL
            ...
            getgenv().AutoFarmConfig.MatchesPlayed = getgenv().AutoFarmConfig.MatchesPlayed + 1
            print("[AUTO FARM LOOP] Game ended - total matches: " .. getgenv().AutoFarmConfig.MatchesPlayed)
            
            -- ✅ PRESIONAR PLAY AGAIN, VOTAR DIFICULTAD, ESPERAR INICIO
            local started = pressPlayAgainAndWait()
            
            if not started then
                warn("[AUTO FARM LOOP] ❌ Failed to start new game - stopping farm")
                stopGlobalTracking()
                WindUI:Notify({
                    Title = "❌ Auto Farm Error",
                    Content = "Could not start new game - farm stopped",
                    Duration = 5
                })
                break
            end
            ...
            -- ✅ RESETEAR TRACKING Y EJECUTAR MACRO
            print("[AUTO FARM LOOP] ========== EXECUTING MACRO FOR MATCH #" .. getgenv().AutoFarmConfig.MatchesPlayed .. " ==========")
            resetGlobalTracking()
            ...
            if not macroSuccess then
                warn("[AUTO FARM LOOP] ❌ Macro error: " .. tostring(macroError))
                WindUI:Notify({
                    Title = "⚠️ Macro Error",
                    Content = "Error in match #" .. getgenv().AutoFarmConfig.MatchesPlayed,
                    Duration = 4
                })
            else
                print("[AUTO FARM LOOP] ✅ Macro complete for match #" .. getgenv().AutoFarmConfig.MatchesPlayed)
            end
        end
