...
local function startAutoFarmLoop(strategyFunction, strategyName)
    task.spawn(function()
        ...
        while getgenv().AutoFarmConfig.IsRunning and (
            ...
            -- ✅ ESPERAR A QUE APAREZCA EL BOTÓN "Play again" Y PRESIONARLO
            print("[AUTO FARM LOOP] ========== WAITING FOR PLAY AGAIN BUTTON ==========")
            task.wait(1.5)
            
            -- Esperar y presionar Play Again
            local playAgainPressed = false
            local waitForButton = 0
            while not playAgainPressed and waitForButton < 20 do
                pcall(function()
                    local gui = PlayerGui:FindFirstChild("GameGui")
                    if gui then
                        for _, button in ipairs(gui:GetDescendants()) do
                            if button:IsA("TextButton") and button.Visible then
                                ...
                                if string.find(text, "again") or string.find(text, "play") then
                                    ...
                                    if conns and #conns > 0 then
                                        ...
                                        print("[AUTO FARM LOOP] ✓ Play Again pressed!")
                                    end
                                end
                            end
                        end
                ...
                if not playAgainPressed then
                    task.wait(0.3)
                    waitForButton = waitForButton + 0.3
                end
            end
            
            if not playAgainPressed then
                warn("[AUTO FARM LOOP] ⚠️ Could not find Play Again button after 20s - trying RemoteFunction")
                pcall(function()
                    game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("RestartGame"):InvokeServer()
                end)
            end
            
            -- ✅ ESPERAR A QUE DESAPAREZCA EL GAME END FRAME (confirma que Play Again funcionó)
            print("[AUTO FARM LOOP] ========== WAITING FOR GAME END FRAME TO CLOSE ==========")
            local endFrameClosed = false
            local waitClose = 0
            while not endFrameClosed and waitClose < 15 do
                pcall(function()
                    local gui = PlayerGui:FindFirstChild("GameGui")
                    if gui then
                        local endFrame = gui.Screen.Middle:FindFirstChild("GameEnd")
                ...
                if not endFrameClosed then
                    task.wait(0.3)
                    waitClose = waitClose + 0.3
                end
            end
            
            if not endFrameClosed then
                warn("[AUTO FARM LOOP] ⚠️ End frame did not close - forcing continue")
            else
                print("[AUTO FARM LOOP] ✓ End frame closed - difficulty selector should appear")
            end
            
            -- ✅ ESPERAR A QUE APAREZCA Y DESAPAREZCA EL SELECTOR DE DIFICULTAD
            task.wait(1)
            local diffSelectorGone = false
            local waitDiff = 0
            while not diffSelectorGone and waitDiff < 20 do
                pcall(function()
                    local gui = PlayerGui:FindFirstChild("GameGui")
                    if gui then
                        -- Votar dificultad mientras el selector esté visible
                        local endFrame = gui.Screen.Middle:FindFirstChild("GameEnd")
                        if endFrame and not endFrame.Visible then
                            -- Intentar votar dificultad
                            pcall(function()
                                game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("PlaceDifficultyVote"):InvokeServer(difficulty)
                            end)
                        end
                ...
                -- Verificar si ya empezó el juego (Wave label visible)
                ...
                if not diffSelectorGone then
                    task.wait(0.5)
                    waitDiff = waitDiff + 0.5
                end
            end
            
            print("[AUTO FARM LOOP] ✓ New game confirmed started after " .. (waitClose + waitDiff) .. "s")
            
            local newGameStarted = diffSelectorGone
            local waitTime = waitClose + waitDiff
            
            if not newGameStarted then
            
            if not newGameStarted then
                warn("[AUTO FARM LOOP] ❌ Failed to confirm new game start")
