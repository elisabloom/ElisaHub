...
task.spawn(function()
    ...
    -- ✅ PASO 3: Usar RestartGame directamente (el botón Play Again no tiene texto detectable)
print("[AUTO FARM LOOP] ⏳ Waiting 3s for end screen to settle...")
task.wait(3) -- Dar tiempo a que aparezca la pantalla de fin completamente

print("[AUTO FARM LOOP] 🔄 Invoking RestartGame...")
local restartSuccess = false
pcall(function()
    game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("RestartGame"):InvokeServer()
    restartSuccess = true
    print("[AUTO FARM LOOP] ✅ RestartGame invoked successfully!")
end)

if not restartSuccess then
    warn("[AUTO FARM LOOP] ❌ RestartGame failed")
end

task.wait(1)
    
    -- ✅ PASO 4: Esperar a que GameEnd desaparezca
print("[AUTO FARM LOOP] ⏳ Waiting for GameEnd to disappear...")
local gameEndGone = false
local waitForGoneTime = 0
local maxWaitForGone = 20
...
print("[AUTO FARM LOOP] ✅ GameEnd gone after " .. waitForGoneTime .. "s")
    
    -- ✅ PASO 5: Esperar que el nuevo match inicie completamente
-- Primero esperar que las Entities existan
print("[AUTO FARM LOOP] ⏳ Waiting for new map/entities...")
local mapReady = false
local waitForMapTime = 0
local maxWaitForMap = 45
...
if not mapReady then
    warn("[AUTO FARM LOOP] ❌ Map never loaded - stopping farm")
    stopGlobalTracking()
    break
end

print("[AUTO FARM LOOP] ✅ Map loaded after " .. waitForMapTime .. "s")

-- Ahora esperar que la pantalla de votación de dificultad desaparezca
-- (el "Choose level" que se ve en pantalla)
print("[AUTO FARM LOOP] ⏳ Waiting for difficulty vote screen to close...")
local voteScreenGone = false
local waitForVoteTime = 0
local maxWaitForVote = 30

while not voteScreenGone and waitForVoteTime < maxWaitForVote and getgenv().AutoFarmConfig.IsRunning do
    pcall(function()
        local gui = PlayerGui:FindFirstChild("GameGui")
        if gui then
            -- Buscar el frame de votación de dificultad
            local voteFrame = gui:FindFirstChild("DifficultyVote", true) 
                or gui:FindFirstChild("VoteFrame", true)
                or gui:FindFirstChild("ChooseLevel", true)
            
            if not voteFrame or not voteFrame.Visible then
                voteScreenGone = true
            end
        else
            voteScreenGone = true
    ...
    if not voteScreenGone then
        task.wait(0.5)
        waitForVoteTime = waitForVoteTime + 0.5
    end
end

-- Buffer final antes de ejecutar el macro
print("[AUTO FARM LOOP] ⏳ Final buffer before macro...")
task.wait(3)
