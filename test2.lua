    ...
    local function plantAndTrack(unitName, getPositionFn, unitDisplayName, expectedCount)
        ...
        for attempt = 1, 50 do
            ...
            if success and result then
                ...
                -- CAPA 2: buscar en workspace directamente
                warn("[VOLCANO] ⚠️ Tracker lento para " .. unitDisplayName .. " - buscando en workspace...")
                task.wait(1.5)
                local model = findUnitByOwnerIndex(expectedCount)
                if model then
                    local id = getIDFromModel(model)
                    if id then
                        if #myUnitIDs < expectedCount then
                            table.insert(myUnitIDs, id)
                        end
                        print("[VOLCANO] ✓ Recovered " .. unitDisplayName .. " via workspace. ID: " .. tostring(id))
                        return id
                    end
                    -- CAPA 3: devolver model como fallback
                    warn("[VOLCANO] ⚠️ Sin ID pero model existe - usando model para " .. unitDisplayName)
                    return model
                end
                warn("[VOLCANO] ⚠️ Planta confirmada pero unit no encontrada - reintentando plant...")
            else
