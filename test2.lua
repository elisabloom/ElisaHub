...
local function runVolcano()
    ...
    if not plantWithRetry("unit_frozen_spike", getGlowthorn2Placement(), "Glowthorn 2") then
        ...
        if #myUnitIDs >= 3 then
            ...
            warn("[VOLCANO] ⚠️ Glowthorn 2 planted but not tracked - continuing without it")

            -- ===== PASO 4 (sin G2): Rafflesia → Lvl 2 =====
            print("[VOLCANO] ========== PASO 4: RAFFLESIA → LVL 2 ==========")
            if not waitForMoney(8000, 60) then return false end
            pcall(function()
                ReplicatedStorage:WaitForChild("RemoteFunctions"):WaitForChild("UpgradeUnit"):InvokeServer(raffID)
            ...
            -- ===== PASO 5 (sin G2): Glowthorn 1 → Lvl 5 =====
            print("[VOLCANO] ========== PASO 5: GLOWTHORN 1 → LVL 5 ==========")
            upgradeToLevel(glowthornID, 5, glowthornCosts, "Glowthorn 1", 4)
        end
    end
    ...
    -- ===== ESPERAR WAVE 17 =====
    print("[VOLCANO] ========== WAITING FOR WAVE 17 ==========")
    local currentWave = 0
    local wave17Detected = false

    while not wave17Detected and getgenv().AutoFarmConfig.VolcanoActive do
        ...
        if wave17Detected then break end
        task.wait(0.5)
    end
