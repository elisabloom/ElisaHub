...
local function runWinter()
    ...
    local function getRandomPositionPath4()
        -- Rango de posiciones válidas para Path 4
        local minPos = Vector3.new(-85.09661102294922, 1.244499921798706, -125.18387603759766)
        local maxPos = Vector3.new(-80.18035888671875, 1.244499921798706, -123.14749908447266)
        ...
        return {
            ...
            PathIndex = 4,
            Position = position,
            CF = CFrame.new(position.X, position.Y, position.Z, 0.3819020390510559, 0, 0.9242027997970581, -0, 1.0000001192092896, -0, -0.9242029190063477, 0, 0.38190197944641113),
            Rotation = 180
        }
    end
...
while getgenv().AutoFarmConfig.IsRunning and (
    ...
    -- ✅ PASO 1: Esperar a que aparezca GameEnd (con timeout de seguridad)
    local gameEnded = false
    local waitForEndTime = 0
    ...
    while not gameEnded and getgenv().AutoFarmConfig.IsRunning do
        ...
        if not gameEnded then
            task.wait(0.5)
            waitForEndTime = waitForEndTime + 0.5
        end
    end
    
    if not getgenv().AutoFarmConfig.IsRunning then break end
    
    print("[AUTO FARM LOOP] ✅ Game ended detected")

    -- DEBUG: listar todos los botones visibles en GameGui
task.spawn(function()
    task.wait(2) -- esperar a que aparezcan los botones
    pcall(function()
        local gui = PlayerGui:FindFirstChild("GameGui")
        if gui then
            print("[DEBUG] === Buttons visible in GameGui ===")
            for _, obj in ipairs(gui:GetDescendants()) do
                if obj:IsA("TextButton") and obj.Visible then
                    print("[DEBUG] Button: '" .. obj.Text .. "' at " .. obj:GetFullName())
                end
            end
            print("[DEBUG] =====================================")
        end
    ...
    -- ✅ PASO 2: Verificar límite de matches
    if getgenv().AntiBanConfig.AutoReturnEnabled and 
       getgenv().AntiBanConfig.MatchesBeforeReturn > 0 and 
       getgenv().AutoFarmConfig.MatchesPlayed >= getgenv().AntiBanConfig.MatchesBeforeReturn then
    ...
    -- ✅ PASO 3: Esperar a que aparezca el botón Play Again y clickearlo
print("[AUTO FARM LOOP] ⏳ Waiting for Play Again button to appear...")
local playAgainClicked = false
local waitForButtonTime = 0
local maxWaitForButton = 30

while not playAgainClicked and waitForButtonTime < maxWaitForButton and getgenv().AutoFarmConfig.IsRunning do
    pcall(function()
        local gui = PlayerGui:FindFirstChild("GameGui")
        if not gui then return end
        
        -- Buscar en TODO el GameGui, no solo en endFrame
        for _, button in ipairs(gui:GetDescendants()) do
            if button:IsA("TextButton") and button.Visible then
                local text = string.lower(button.Text)
                if string.find(text, "play again") or text == "play again" then
                    print("[AUTO FARM LOOP] 🎯 Found Play Again button: '" .. button.Text .. "' at " .. button:GetFullName())
                    
                    -- Intentar getconnections primero
                    local conns = getconnections(button.MouseButton1Click)
                    if conns and #conns > 0 then
                        conns[1]:Fire()
                        playAgainClicked = true
                        print("[AUTO FARM LOOP] ✅ Play Again clicked via getconnections!")
                        return
                    end
                    
                    -- Fallback: FireClickDetector o MouseClick
                    local clickDetector = button:FindFirstChildOfClass("ClickDetector")
                    if clickDetector then
                        fireclickdetector(clickDetector)
                        playAgainClicked = true
                        print("[AUTO FARM LOOP] ✅ Play Again clicked via ClickDetector!")
                        return
                    end
                    
                    -- Último fallback: simular el evento directamente
                    button.MouseButton1Click:Fire()
                    playAgainClicked = true
                    print("[AUTO FARM LOOP] ✅ Play Again clicked via direct Fire!")
                    return
                end
            end
        end
    ...
    if not playAgainClicked then
        task.wait(0.3)
        waitForButtonTime = waitForButtonTime + 0.3
        
        -- Log cada 5 segundos para debug
        if math.floor(waitForButtonTime * 10) % 50 == 0 then
            print("[AUTO FARM LOOP] Still searching for Play Again... (" .. waitForButtonTime .. "s)")
        end
    end
end

if not playAgainClicked then
    warn("[AUTO FARM LOOP] ⚠️ Play Again not found after " .. maxWaitForButton .. "s - trying RemoteFunction fallback")
    pcall(function()
        game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("RestartGame"):InvokeServer()
    end)
    task.wait(2)
    ...
    -- ✅ PASO 4: Esperar a que GameEnd DESAPAREZCA completamente (confirma que Play Again funcionó)
    print("[AUTO FARM LOOP] ⏳ Waiting for GameEnd to disappear...")
    local gameEndGone = false
    local waitForGoneTime = 0
    local maxWaitForGone = 30
    
    while not gameEndGone and waitForGoneTime < maxWaitForGone and getgenv().AutoFarmConfig.IsRunning do
        pcall(function()
            local gui = PlayerGui:FindFirstChild("GameGui")
            if gui then
                local endFrame = gui.Screen.Middle:FindFirstChild("GameEnd")
                if not endFrame or not endFrame.Visible then
                    gameEndGone = true
                end
            else
                gameEndGone = true -- GUI no existe, asumimos que transitó
            end
        end)
        
        if not gameEndGone then
            task.wait(0.5)
            waitForGoneTime = waitForGoneTime + 0.5
        end
    end
    
    if not gameEndGone then
        warn("[AUTO FARM LOOP] ❌ GameEnd never disappeared after " .. maxWaitForGone .. "s - stopping farm")
        stopGlobalTracking()
        WindUI:Notify({
            Title = "❌ Auto Farm Error",
            Content = "Game end screen stuck - farm stopped",
            Duration = 5
        })
        break
    end
    
    print("[AUTO FARM LOOP] ✅ GameEnd disappeared after " .. waitForGoneTime .. "s")
    
    -- ✅ PASO 5: Esperar que el nuevo mapa cargue completamente
    -- Detectar que las Entities existen y están listas
    print("[AUTO FARM LOOP] ⏳ Waiting for new map to load...")
    local mapReady = false
    local waitForMapTime = 0
    local maxWaitForMap = 30
    
    while not mapReady and waitForMapTime < maxWaitForMap and getgenv().AutoFarmConfig.IsRunning do
        pcall(function()
            local mapFolder = Workspace:FindFirstChild("Map")
            if mapFolder then
                local entities = mapFolder:FindFirstChild("Entities")
        ...
        if not mapReady then
            task.wait(0.5)
            waitForMapTime = waitForMapTime + 0.5
        end
    end
    
    if not mapReady then
        warn("[AUTO FARM LOOP] ❌ Map/Entities never loaded after " .. maxWaitForMap .. "s - stopping farm")
        stopGlobalTracking()
        WindUI:Notify({
            Title = "❌ Auto Farm Error",
            Content = "Map did not load - farm stopped",
            Duration = 5
        })
        break
    end
    
    -- ✅ PASO 6: Buffer extra para que el juego inicialice completamente
    task.wait(2)
    
    if not getgenv().AutoFarmConfig.IsRunning then break end
    
    -- ✅ PASO 7: Resetear tracking para nueva partida
    print("[AUTO FARM LOOP] 🔄 Resetting tracking for Match #" .. (getgenv().AutoFarmConfig.MatchesPlayed + 1))
    resetGlobalTracking()
    
    -- ✅ PASO 8: Votar dificultad AHORA (mapa ya cargado, antes no había mapa)
