...
local function startAutoFarmLoop(strategyFunction, strategyName)
    task.spawn(function()
        ...
        while getgenv().AutoFarmConfig.IsRunning and (
            ...
            -- Esperar dinamicamente a que desaparezca GameEnd (Play Again presionado)
            ...
            print("[AUTO FARM LOOP] ✓ GameEnd gone - waiting for wave to spawn")
            
            -- Votar dificultad ahora que Play Again fue presionado
            pcall(function()
                game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("PlaceDifficultyVote"):InvokeServer(difficulty)
            end)
            
            -- Esperar dinamicamente a que la wave spawne (Cash visible = juego iniciado)
            repeat task.wait(0.5) until (function()
                local ready = false
                pcall(function()
                    local gui = PlayerGui:FindFirstChild("GameGui")
                    if gui then
                        ...
                        if cashLabel and cashLabel.Visible then
                            ready = true
                        end
                    end
                end)
            ...
            print("[AUTO FARM LOOP] ✓ Wave spawned - executing macro")
