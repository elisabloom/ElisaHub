...
-- ==================== SISTEMA DE DETECCIÓN Y SELECCIÓN DE MAPAS ====================
local MapSelectorConfig = {
    -- Selectores de mapa disponibles (en orden de prioridad)
    Selectors = {
        {
            name = "Selector 1 (0/4)",
            position = CFrame.new(118.89, 78, 779.65),
            maxPlayersRemote = "LobbySetMaxPlayers_8",
        ...
        {
            name = "Selector 2 (0/3)",
            position = CFrame.new(108.79, 72.21, 776.50),
            maxPlayersRemote = "LobbySetMaxPlayers_7",
    ...
    MaxRetries = 3,
    RetryDelay = 5,
    SafeExitPosition = CFrame.new(118.89, 78, 750)
}

local function countPlayersInSelector(selectorPosition, radius)
    local count = 0
    pcall(function()
        for _, player in pairs(Players:GetPlayers()) do
            if player ~= LocalPlayer and player.Character then
                ...
                if hrp then
                    local distance = (hrp.Position - selectorPosition.Position).Magnitude
                    if distance < radius then
                        count = count + 1
                        print("[MAP SELECTOR] Detected player: " .. player.Name .. " (distance: " .. math.floor(distance) .. " studs)")
                    end
                end
            end
        end
...
local function exitMapSelector(leaveRemote)
    print("[MAP SELECTOR] Exiting selector using: " .. leaveRemote)
    
    local success = pcall(function()
        game:GetService("ReplicatedStorage")
            :WaitForChild("RemoteFunctions")
            :WaitForChild(leaveRemote)
            :InvokeServer()
    end)
    
    if success then
        print("[MAP SELECTOR] ✓ Successfully exited selector")
        task.wait(0.5)
        
        local Character = LocalPlayer.Character
        if Character and Character:FindFirstChild("HumanoidRootPart") then
            Character.HumanoidRootPart.CFrame = MapSelectorConfig.SafeExitPosition
            print("[MAP SELECTOR] ✓ Moved to safe position")
        end
    else
        warn("[MAP SELECTOR] Failed to use leave remote, using fallback teleport")
        local Character = LocalPlayer.Character
        if Character and Character:FindFirstChild("HumanoidRootPart") then
            Character.HumanoidRootPart.CFrame = MapSelectorConfig.SafeExitPosition
        end
...
local function setupMapSelector(mapName, maxAttempts)
    maxAttempts = maxAttempts or MapSelectorConfig.MaxRetries
    
    print("[MAP SELECTOR] ========== STARTING MAP SETUP ==========")
    print("[MAP SELECTOR] Target map: " .. mapName)
    print("[MAP SELECTOR] Max attempts: " .. maxAttempts)
    
    for attempt = 1, maxAttempts do
        print("[MAP SELECTOR] ===== ATTEMPT " .. attempt .. "/" .. maxAttempts .. " =====")
        
        for selectorIndex, selector in ipairs(MapSelectorConfig.Selectors) do
            print("[MAP SELECTOR] Testing " .. selector.name .. "...")
            
            local Character = LocalPlayer.Character
            if not Character or not Character:FindFirstChild("HumanoidRootPart") then
                warn("[MAP SELECTOR] Character or HRP not found!")
                continue
            end
            
            local HRP = Character.HumanoidRootPart
            HRP.CFrame = selector.position
            print("[MAP SELECTOR] ✓ Teleported to " .. selector.name)
            task.wait(1.5)
            
            local playersNearby = countPlayersInSelector(selector.position, selector.detectionRadius)
            
            if playersNearby > 0 then
                print("[MAP SELECTOR] ⚠️ " .. selector.name .. " is OCCUPIED (" .. playersNearby .. " players detected)")
                print("[MAP SELECTOR] Skipping to next selector...")
                continue
            end
            
            print("[MAP SELECTOR] ✅ " .. selector.name .. " is EMPTY!")
            print("[MAP SELECTOR] Configuring map settings...")
            
            print("[MAP SELECTOR] Setting max players to 1 using: " .. selector.maxPlayersRemote)
            local success1, error1 = pcall(function()
                game:GetService("ReplicatedStorage")
                    :WaitForChild("RemoteFunctions")
                    :WaitForChild(selector.maxPlayersRemote)
                    :InvokeServer(1)
            end)
            
            if not success1 then
                warn("[MAP SELECTOR] ❌ Failed to set max players: " .. tostring(error1))
                exitMapSelector(selector.leaveRemote)
                task.wait(1)
                continue
            end
            
            print("[MAP SELECTOR] ✓ Max players set to 1")
            task.wait(0.4)
            
            print("[MAP SELECTOR] Setting map to: " .. mapName .. " using: " .. selector.setMapRemote)
            local success2, error2 = pcall(function()
                game:GetService("ReplicatedStorage")
                    :WaitForChild("RemoteFunctions")
                    :WaitForChild(selector.setMapRemote)
                    :InvokeServer(mapName)
            end)
            
            if not success2 then
                warn("[MAP SELECTOR] ❌ Failed to set map: " .. tostring(error2))
                exitMapSelector(selector.leaveRemote)
                task.wait(1)
                continue
            end
            
            print("[MAP SELECTOR] ✓ Map set to: " .. mapName)
            print("[MAP SELECTOR] ========== ✅✅✅ SETUP COMPLETE ✅✅✅ ==========")
            print("[MAP SELECTOR] Used: " .. selector.name)
            return true
        end
        
        if attempt < maxAttempts then
            print("[MAP SELECTOR] ⚠️ All selectors failed or occupied in attempt " .. attempt)
            print("[MAP SELECTOR] Waiting " .. MapSelectorConfig.RetryDelay .. " seconds before retry...")
            task.wait(MapSelectorConfig.RetryDelay)
        end
    end
    
    warn("[MAP SELECTOR] ========== ❌ SETUP FAILED ❌ ==========")
    warn("[MAP SELECTOR] All attempts exhausted after " .. maxAttempts .. " tries")
    return false
end
        ...
        Callback = function(state)
            if state then
                ...
                if currentMap == "map_lobby" then
                    WindUI:Notify({Title = "Lobby Setup", Content = "Setting up Graveyard...", Duration = 3})
                    
                    task.spawn(function()
                        local setupSuccess = setupMapSelector("map_graveyard", 3)
                        ...
                        if setupSuccess then
                            ...
                            WindUI:Notify({
                                Title = "Setup Failed", 
                                Content = "Could not configure map. Try again later.", 
                                Duration = 5
                            })
                        end
                    end)
...
-- ✅ GUARDAR REFERENCIA GLOBAL
    ...
    local DojoToggle = AutoFarmTab:Toggle({
        ...
        Callback = function(state)
            if state then
                if getgenv().AutoFarmConfig.GraveyardV1Active or getgenv().AutoFarmConfig.GraveyardV2Active or getgenv().AutoFarmConfig.AutoWinV1Active or getgenv().AutoFarmConfig.AutoWinV2Active then
                    WindUI:Notify({
                        Title = "Error",
                ...
                if currentMap == "map_lobby" then
                    print("[DOJO] Detected in lobby - Starting setup...")
                    
                    WindUI:Notify({
                        Title = "Lobby Setup",
                        Content = "Setting up Dojo...",
                        Duration = 3
                    })
                    
                    task.spawn(function()
                        local setupSuccess = setupMapSelector("map_dojo", 3)
...
local AutoWinV2Toggle = AutoFarmTab:Toggle({
    ...
    Callback = function(state)
        if state then
            ...
            if currentMap == "map_lobby" then
                print("[AUTO WIN V2] Detected in lobby - Starting setup...")
                
                WindUI:Notify({
                    Title = "Lobby Setup",
                    Content = "Setting up Garden...",
                    Duration = 3
                })
                
                task.spawn(function()
                    local setupSuccess = setupMapSelector("map_farm", 3)
