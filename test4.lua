...
-- ==================== FUNCIÓN PARA DETECTAR JUGADORES EN BOOTH ====================
local function isBoothOccupied(boothPosition)
    local success, result = pcall(function()
        ...
        for _, player in pairs(Players:GetPlayers()) do
            if player ~= LocalPlayer and player.Character then
                ...
                if hrp then
                    local distance = (hrp.Position - boothPosition).Magnitude
                    if distance < 15 then -- Radio de 15 studs
...
-- ==================== FUNCIÓN PARA DETECTAR SI EL BOOTH ESTÁ OCUPADO ====================
local function isBoothOccupied(boothType)
    local success, result = pcall(function()
        -- Verificar distancia física (método más confiable)
        local boothPositions = {
            ["8"] = Vector3.new(118.89, 78, 779.65),
            ["7"] = Vector3.new(108.79, 72.21, 776.50)
        }
        
        local targetPos = boothPositions[boothType]
        ...
        for _, player in pairs(Players:GetPlayers()) do
            if player ~= LocalPlayer and player.Character then
                ...
                if hrp then
                    ...
                    -- Radio de 12 studs para detectar jugadores
                    if distance < 12 then
                        playersNearby = playersNearby + 1
                        print("[BOOTH CHECK] Player '" .. player.Name .. "' detected at distance: " .. math.floor(distance) .. " studs")
                    end
                end
            end
        end
    ...
    if not success then
        warn("[BOOTH CHECK] Error checking booth: " .. tostring(result))
        return false
    end
...
task.spawn(function()
   ...
   local MapConfig = {
    ["map_farm"] = {
        teleport_primary = CFrame.new(118.89, 78, 779.65),
        teleport_secondary = CFrame.new(108.79, 72.21, 776.50),
        remote_primary = "LobbySetMaxPlayers_8",
        remote_secondary = "LobbySetMaxPlayers_7",
        leave_primary = "LeaveLobby_8",
        leave_secondary = "LeaveLobby_7"
    },
    ["map_jungle"] = {
        teleport_primary = CFrame.new(118.89, 78, 779.65),
        teleport_secondary = CFrame.new(108.79, 72.21, 776.50),
    ...
    AutoJoinMapToggle = MainTab:Toggle({
        ...
        Callback = function(state)
            if state then
                ...
                -- ✅ SISTEMA DE REINTENTO CON BOOTH ALTERNATIVO
                task.spawn(function()
                    local success = false
                    local maxRetries = 3
                    local retryDelay = 3
                    ...
                    for attempt = 1, maxRetries do
                        print("[AUTO JOIN MAP] 🔄 Attempt " .. attempt .. "/" .. maxRetries)
                        
                        local usePrimary = true
                        local Character = LocalPlayer.Character
                        
                        if not Character then
                            warn("[AUTO JOIN MAP] Character not found")
                            break
                        end
                        
                        local HumanoidRootPart = Character:FindFirstChild("HumanoidRootPart")
                        if not HumanoidRootPart then
                            warn("[AUTO JOIN MAP] HumanoidRootPart not found")
                            break
                        end
                        
                        -- STEP 1: Teleport a Primary booth
                        print("[AUTO JOIN MAP] 📍 Teleporting to Primary booth...")
                        HumanoidRootPart.CFrame = mapConfig.teleport_primary
                        task.wait(1.5)
                        
                        -- STEP 2: Verificar si hay personas en Primary
                        task.wait(0.5) -- Esperar a que cargue el GUI
                        if isBoothOccupied("8") then
                            print("[AUTO JOIN MAP] ⚠️ Primary booth occupied! Trying Secondary...")
                            
                            WindUI:Notify({
                                Title = "Booth Occupied",
                                Content = "Trying alternative booth...",
                                Duration = 2
                            })
                            
                            -- Salir del Primary booth
                            pcall(function()
                                game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild(mapConfig.leave_primary):InvokeServer()
                            end)
                            task.wait(0.5)
                            
                            -- STEP 3: Teleport a Secondary booth
                            print("[AUTO JOIN MAP] 📍 Teleporting to Secondary booth...")
                            HumanoidRootPart.CFrame = mapConfig.teleport_secondary
                            task.wait(1.5)
                            
                            -- STEP 4: Verificar si Secondary también está ocupado
                            task.wait(0.5) -- Esperar a que cargue el GUI
                            if isBoothOccupied("7") then
                                print("[AUTO JOIN MAP] ⚠️ Secondary booth also occupied!")
                                
                                WindUI:Notify({
                                    Title = "Both Booths Occupied",
                                    Content = "Retrying in " .. retryDelay .. " seconds...",
                                    Duration = 2
                                })
                                
                                -- Salir del Secondary booth
                                pcall(function()
                                    game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild(mapConfig.leave_secondary):InvokeServer()
                                end)
                                ...
                                usePrimary = false
                                print("[AUTO JOIN MAP] ✅ Secondary booth available!")
                            end
                        else
                            print("[AUTO JOIN MAP] ✅ Primary booth available!")
                        end
                        
                        -- STEP 5: Configurar el mapa
                        local configSuccess, configError = pcall(function()
                            local maxPlayersRemote = usePrimary and mapConfig.remote_primary or mapConfig.remote_secondary
                            local setMapRemote = usePrimary and mapConfig.setmap_primary or mapConfig.setmap_secondary
                            
                            print("[AUTO JOIN MAP] 🎮 Using: " .. (usePrimary and "Primary" or "Secondary") .. " booth")
                            print("[AUTO JOIN MAP] Setting max players to 1...")
                            
                            local args = { 1 }
                            local success1 = game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild(maxPlayersRemote):InvokeServer(unpack(args))
                            print("[AUTO JOIN MAP] SetMaxPlayers: " .. tostring(success1))
                            task.wait(0.3)
                            
                            print("[AUTO JOIN MAP] Setting map to: " .. getgenv().MainTabConfig.SelectedMap)
                            local args2 = { getgenv().MainTabConfig.SelectedMap }
                            local success2 = game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild(setMapRemote):InvokeServer(unpack(args2))
                            print("[AUTO JOIN MAP] SetMap: " .. tostring(success2))
                        ...
                        if configSuccess and success then
                            print("[AUTO JOIN MAP] ✅ Successfully configured!")
                            WindUI:Notify({
                                Title = "Auto Join Map",
                            ...
                            warn("[AUTO JOIN MAP] ❌ Configuration failed: " .. tostring(configError))
                            
                            -- Salir del booth actual
                            local leaveRemote = usePrimary and mapConfig.leave_primary or mapConfig.leave_secondary
                            pcall(function()
                                game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild(leaveRemote):InvokeServer()
                            end)
                            
                            if attempt < maxRetries then
                                print("[AUTO JOIN MAP] Retrying in " .. retryDelay .. " seconds...")
                                task.wait(retryDelay)
                            end
                        end
                    end
                    ...
                    if not success then
                        WindUI:Notify({
                            Title = "Error",
                            Content = "Failed after " .. maxRetries .. " attempts. Booths may be full.",
                            Duration = 5
                        })
                        warn("[AUTO JOIN MAP] All attempts exhausted")
                    end
                ...
                if currentMap == "map_lobby" then
                    ...
                    task.spawn(function()
                        ...
                        if Character then
                            ...
                            if HRP then
                                print("[GRAVEYARD V2] Teleporting to lobby position...")
                                HRP.CFrame = CFrame.new(118.89, 78, 779.65)
                                task.wait(1)
                                print("[GRAVEYARD V2] ✓ Teleported successfully")
                            end
                        end
                        
                        print("[GRAVEYARD V2] Setting max players to 1...")
                        local success1 = pcall(function()
                            game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("LobbySetMaxPlayers_8"):InvokeServer(1)
                        end)
                        print("[GRAVEYARD V2] SetMaxPlayers result: " .. tostring(success1))
                        task.wait(0.3)
                        
                        print("[GRAVEYARD V2] Setting map to Graveyard...")
                        local success2 = pcall(function()
                            game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("LobbySetMap_8"):InvokeServer("map_graveyard")
                        end)
                        print("[GRAVEYARD V2] SetMap result: " .. tostring(success2))
                    ...
                    task.spawn(function()
                        ...
                        if Character then
                            ...
                            if HumanoidRootPart then
                                print("[DOJO] Teleporting to lobby position...")
                                HumanoidRootPart.CFrame = CFrame.new(118.89, 78, 779.65)
                                task.wait(1)
                                print("[DOJO] ✓ Teleported successfully")
                            end
                        end
                        
                        print("[DOJO] Setting max players to 1...")
                        local success1 = pcall(function()
                            local args = { 1 }
                            game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("LobbySetMaxPlayers_8"):InvokeServer(unpack(args))
                        end)
                        print("[DOJO] SetMaxPlayers result: " .. tostring(success1))
                        task.wait(0.3)
                        
                        print("[DOJO] Setting map to Dojo...")
                        local success2 = pcall(function()
                            local args = { "map_dojo" }
                            game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("LobbySetMap_8"):InvokeServer(unpack(args))
                        end)
                        print("[DOJO] SetMap result: " .. tostring(success2))
            ...
            if currentMap == "map_lobby" then
                ...
                task.spawn(function()
                    ...
                    if Character then
                        local HumanoidRootPart = Character:FindFirstChild("HumanoidRootPart")
                        if HumanoidRootPart then
                            print("[AUTO WIN V1] Teleporting to lobby position...")
                            HumanoidRootPart.CFrame = CFrame.new(118.89, 78, 779.65)
                            task.wait(1)
                            print("[AUTO WIN V1] ✓ Teleported successfully")
                        end
                    end
                    
                    print("[AUTO WIN V1] Setting max players to 1...")
                    local success1 = pcall(function()
                        local args = { 1 }
                        game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("LobbySetMaxPlayers_8"):InvokeServer(unpack(args))
                    end)
                    print("[AUTO WIN V1] SetMaxPlayers result: " .. tostring(success1))
                    task.wait(0.3)
                    
                    print("[AUTO WIN V1] Setting map to Garden...")
                    local success2 = pcall(function()
                        local args = { "map_farm" }
                        game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("LobbySetMap_8"):InvokeServer(unpack(args))
                    end)
                    print("[AUTO WIN V1] SetMap result: " .. tostring(success2))
            ...
            if currentMap == "map_lobby" then
                ...
                WindUI:Notify({
                    ...
                    if Character then
                        local HumanoidRootPart = Character:FindFirstChild("HumanoidRootPart")
                        if HumanoidRootPart then
                            print("[AUTO WIN V2] Teleporting to lobby position...")
                            HumanoidRootPart.CFrame = CFrame.new(118.89, 78, 779.65)
                            task.wait(1)
                            print("[AUTO WIN V2] ✓ Teleported successfully")
                        end
                    end
                    
                    print("[AUTO WIN V2] Setting max players to 1...")
                    local success1 = pcall(function()
                        local args = { 1 }
                        game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("LobbySetMaxPlayers_8"):InvokeServer(unpack(args))
                    end)
                    print("[AUTO WIN V2] SetMaxPlayers result: " .. tostring(success1))
                    task.wait(0.3)
                    
                    print("[AUTO WIN V2] Setting map to Garden...")
                    local success2 = pcall(function()
                        local args = { "map_farm" }
                        game:GetService("ReplicatedStorage"):WaitForChild("RemoteFunctions"):WaitForChild("LobbySetMap_8"):InvokeServer(unpack(args))
                    end)
                    print("[AUTO WIN V2] SetMap result: " .. tostring(success2))
