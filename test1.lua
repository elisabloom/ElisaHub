...
local function runDojo()
    ...
    -- ===== FUNCIÓN DE TRACKING CON SIMULACIÓN DE FALLO =====
    local function waitForTracking(expectedCount, unitName, maxWait)
        maxWait = maxWait or 5
        local waitTime = 0
        
        -- 🔴 SIMULAR FALLO EN RAFFLESIA 2 SOLO EN PARTIDAS 1 Y 2
    ...
    if not plantRafflesiaWithRetry(getRandomPositionPath1, "Rafflesia 1", 10) then
        warn("[DOJO] Failed to plant Rafflesia 1")
        goto skip_match
    end
    ...
    if not plantRafflesiaWithRetry(getRandomPositionPath2, "Rafflesia 2", 10) then
        warn("[DOJO] Failed to plant Rafflesia 2")
        goto skip_match
    end
