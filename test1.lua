...
-- ==================== DOJO: FUNCIONES DE POSICIÓN RANDOM CON OFFSET ====================
local function getRandomPositionPath1()
    ...
    local pathWidth = math.abs(maxPos.X - minPos.X)
    local pathLength = math.abs(maxPos.Z - minPos.Z)
    local safeOffset = math.min(offset, pathWidth / 4, pathLength / 4)
    
    local offsetX = (math.random() - 0.5) * 2 * safeOffset
    local offsetZ = (math.random() - 0.5) * 2 * safeOffset
    
    -- ✅ FIX: Usar math.min/math.max para asegurar orden correcto
    local minX = math.min(minPos.X, maxPos.X)
    local maxX = math.max(minPos.X, maxPos.X)
    local minZ = math.min(minPos.Z, maxPos.Z)
    local maxZ = math.max(minPos.Z, maxPos.Z)
    
    randomX = math.clamp(randomX + offsetX, minX, maxX)
    randomZ = math.clamp(randomZ + offsetZ, minZ, maxZ)
...
local function plantRafflesiaWithRetry(pathFunction, rafflesiaName, maxAttempts)
    ...
    for attempt = 1, maxAttempts do
        ...
        if success then 
            print("[DOJO] ✓ Placed " .. rafflesiaName .. " on attempt " .. attempt .. " (offset: " .. (getgenv().AntiBanConfig.PlacementOffset or 1.5) .. " studs)")
            return true
        end
...
local function runDojo()
    ...
    if not plantRafflesiaWithRetry(getRandomPositionPath1, "Rafflesia 1", 50) then
        WindUI:Notify({
            Title = "Dojo Error",
            Content = "Failed to plant Rafflesia 1 after 50 attempts",
            Duration = 5
        })
        return false
    end
    ...
    if #myUnitIDs < 1 then
        warn("[DOJO] Failed to track Rafflesia 1!")
        WindUI:Notify({
            Title = "Dojo Tracking Error",
            Content = "Rafflesia 1 not detected - stopping",
            Duration = 5
        })
        return false
    end
    ...
    if not plantRafflesiaWithRetry(getRandomPositionPath2, "Rafflesia 2", 50) then
        WindUI:Notify({
            Title = "Dojo Error",
            Content = "Failed to plant Rafflesia 2 after 50 attempts",
            Duration = 5
        })
        return false
    end
    ...
    if #myUnitIDs < 2 then
        ...
        WindUI:Notify({
            Title = "Dojo Tracking Error",
            Content = "Rafflesia 2 not detected - stopping",
            Duration = 5
        })
        return false
    end
    ...
    if not raff1ID or not raff2ID then
        warn("[DOJO CRITICAL] One or both IDs are nil!")
        return false
    end
