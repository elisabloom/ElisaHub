...
local function getRandomPositionPath1()
    ...
    randomX = math.clamp(randomX, minX, maxX)
    randomZ = math.clamp(randomZ, minZ, maxZ)
...
-- ✅ VERSIÓN MEJORADA: Solo 10 reintentos con wait más largo
local function plantRafflesiaWithRetry(pathFunction, rafflesiaName, maxAttempts)
    maxAttempts = maxAttempts or 10  -- ✅ CAMBIO: 50 → 10 intentos
    ...
    for attempt = 1, maxAttempts do
        ...
        if success and result then  -- ✅ Verificar que result sea true
            print("[DOJO] ✓ Placed " .. rafflesiaName .. " on attempt " .. attempt)
            return true
        end
        
        task.wait(0.3)  -- ✅ CAMBIO: 0.05 → 0.3 segundos (más humano)
    end
...
local function runDojo()
    ...
    task.wait(1)  -- ✅ Wait más largo para tracking
