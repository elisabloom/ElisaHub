...
local function runDojo()
    ...
    local failReason = "Unknown error"
    
    -- ===== SIMULADOR DE FALLO (SOLO PARA TESTING) =====
    local currentMatchNumber = getgenv().AutoFarmConfig.MatchesPlayed + 1
    local shouldFailThisMatch = (currentMatchNumber == 1 or currentMatchNumber == 2)
    
    if shouldFailThisMatch then
        print("[DOJO TEST] WARNING SIMULATING FAILURE IN MATCH " .. currentMatchNumber)
    end
    
    -- ===== FUNCION DE TRACKING CON TIMEOUT Y FALLO SIMULADO =====
    local function waitForTracking(expectedCount, unitName, maxWait)
        maxWait = maxWait or 5
        local waitTime = 0
        
        -- ===== FALLO FORZADO PARA TESTING (solo en Rafflesia 2) =====
        if shouldFailThisMatch and unitName == "Rafflesia 2" then
            print("[DOJO TEST] FORCING TRACKING FAILURE FOR " .. unitName)
            macroFailed = true
            failReason = "Simulated tracking failure for " .. unitName
            return false
        end
        
        while #myUnitIDs < expectedCount and waitTime < maxWait do
            task.wait(0.2)
            waitTime = waitTime + 0.2
        end
        
        if #myUnitIDs < expectedCount then
            warn("[DOJO] Failed to track " .. unitName .. " after " .. waitTime .. " seconds!")
            macroFailed = true
            failReason = "Failed to track " .. unitName
            return false
        end
        
        print("[DOJO] " .. unitName .. " tracked successfully ID: " .. myUnitIDs[expectedCount])
        return true
    end
    ...
    if not waitForTracking(1, "Rafflesia 1") then 
        goto skip_match 
    end
    ...
    -- ===== AQUI ES DONDE FORZAMOS EL FALLO EN MATCH 1 Y 2 =====
    if not waitForTracking(2, "Rafflesia 2") then 
        goto skip_match 
    end
    ...
    print("[SUCCESS] MATCH " .. currentMatchNumber .. " COMPLETED SUCCESSFULLY")
    return true
    
    -- ===== LABEL PARA SALTAR MATCH FALLIDO =====
    ::skip_match::
    warn("==========================================")
    warn("[DOJO] MATCH " .. currentMatchNumber .. " FAILED!")
    warn("[DOJO] Reason: " .. failReason)
...
### **Match 1:**
```
[DOJO TEST] WARNING SIMULATING FAILURE IN MATCH 1
[DOJO] Planting Rafflesia 1 Path 1
[DOJO] Placed Rafflesia 1 on attempt 1
[DOJO] Rafflesia 1 tracked successfully ID: 1
[DOJO] Planting Rafflesia 2 Path 2
[DOJO] Placed Rafflesia 2 on attempt 1
[DOJO TEST] FORCING TRACKING FAILURE FOR Rafflesia 2
==========================================
[DOJO] MATCH 1 FAILED!
[DOJO] Reason: Simulated tracking failure for Rafflesia 2
[DOJO] Waiting for game end
==========================================
```

### **Match 2:**
(Mismo comportamiento que Match 1)
...
[DOJO] Planting Rafflesia 2 Path 2
[DOJO] Placed Rafflesia 2 on attempt 1
[DOJO] Rafflesia 2 tracked successfully ID: 2
[DOJO] Upgrading Rafflesia 1
[DOJO] Upgrading Rafflesia 2
[DOJO] WAVE 10 REACHED!
[SUCCESS] MATCH 3 COMPLETED SUCCESSFULLY
