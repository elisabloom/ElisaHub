–// Discord Webhook Tracker - Complete Fix v2
...
for _, descendant in pairs(gameGui:GetDescendants()) do
    if descendant:IsA("TextLabel") then
        ...
        -- El GUI muestra algo como "🍬 470" o simplemente "470" cerca del icono
...
local success, err = pcall(function()
    ...
    -- NUEVO: Obtener Candy del GUI del juego
    local totalCandy = getCandyFromGameGui()
    warn("[DEBUG] Candy Value (from GUI): " .. totalCandy)
    
    -- Buscar el resultado del juego de forma más exhaustiva
    ...
    -- Método 2: Buscar en TODOS los TextLabels del GameEnd frame
    if resultText == "Unknown" then
        for _, child in pairs(gameEndFrame:GetDescendants()) do
            if child:IsA("TextLabel") then
                local text = child.Text:lower()
                
                -- Buscar palabras clave de victoria
                ...
                -- Buscar palabras clave de derrota
    ...
    for _, child in pairs(gameEndFrame:GetDescendants()) do
        if child:IsA("TextLabel") then
            ...
            -- Buscar formato de tiempo XX:XX o X:XX
...
warn(”========================================”)
warn(”[WEBHOOK TRACKER] Initialized v2 (Complete Fix)”)
...
warn(”[INFO] Features: Candy GUI detection + Win/Loss detection”)
warn(”========================================”)
